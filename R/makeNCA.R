...
makeNCA <- function(x, postPred = F, include, exclude, input = 1, icen = "median", outeq = 1, block = 1,
    ...
    if (is.numeric(x)) { # run number
      ...
      if (length(ls(pattern = NPdataName)) > 0 | length(ls(pattern = ITdataName)) > 0) { # objects are already loaded
        ...
        if (loadOK) { # load was ok
          ...
          stop("Please ensure that current working directory is your Runs folder and run exists.\n")
        }
      }
    } else { # end is numeric x
      if (inherits(x, "PM_data")) {
        mdata <- x$standard_data
      } else if (inherits(x, "PM_result")) {
        mdata <- x$data$standard_data
      } else if (inherits(x, "PMmatrix")) {
        mdata <- x # mdata file specified directly
      } else {
        mdata <- PMreadMatrix(x)
      } # x must be a character vector of file to load
  ...
  timeFilter <- function(id, thisStartTime, thisEndTime) {
    dataSub <- mdata[mdata$id == id & mdata$time >= thisStartTime & mdata$time <= thisEndTime, ]
    return(dataSub)
  }

  timeFilterPost <- function(id, thisStartTime, thisEndTime) {
    dataSub <- post[post$id == id & post$time >= thisStartTime & post$time <= thisEndTime, ]
    return(dataSub)
  }
  ...
  conv_post <- function(post, mdata, include, exclude, input, outeq, block, icen, start, end, first, last) {
    ...
    post2 <- mapply(timeFilterPost, id = unique(post$id), thisStartTime = startTimes, thisEndTime = endTimes, SIMPLIFY = F)
