...
make_NCA <- function(x, postPred = F, include, exclude, input = 1, icen = "median", outeq = 1, block = 1,
  ...
  mdataFilter <- function(mdata, start, end, first, last) {
    ...
    mdata2 <- purrr::pmap(.l = list(idkeep = unique(mdata$id), thisStartTime = startTimes, 
                                    thisEndTime = endTimes), .f = timeFilter) %>%
      dplyr::bind_rows()
  ...
  conv_post <- function(post, mdata, include, exclude, input, outeq, block, icen, start, end, first, last) {
    ...
    post2 <- purrr::pmap(.l = list(idkeep = unique(post$id), thisStartTime = startTimes, 
                                    thisEndTime = endTimes), .f = timeFilterPost) %>%
      dplyr::bind_rows()
