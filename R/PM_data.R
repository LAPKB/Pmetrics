...
plot.PM_data <- function(
  ...
  if (lifecycle::is_present(block)) {
    ...
    lifecycle::deprecate_warn("3.3.0", "plot.PM_data(block)", "plot.PM_data(occasion)")
    occasion <- block
  }
  ...
  if (!is.null(marker$color)) {
    marker$color <- as.character(marker$color)
    has_hex <- stringr::str_detect(marker$color, "#")
    if (any(has_hex, na.rm = TRUE)) {
      alpha_val <- if (is.null(marker$opacity)) 1 else marker$opacity[[1]]
      marker$color[has_hex] <- vapply(
        marker$color[has_hex],
        rgba_to_rgb,
