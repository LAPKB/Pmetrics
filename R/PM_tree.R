...
PM_tree <- function(project = "NewProject", path = getwd()) {
  ...
  writeLines(
    c(
      ...
      "# Option 1:",
      glue::glue("setwd(\"{newFolder}\") # change the working directory"),
      "# Option 2:",
      glue::glue("root <- \"{newFolder}\" # name the project root folder"),
      "",
      "# Ensure your data file is in the /src folder",
      "",
      "# Make the data object",
      "# Option 1:",
      "dat <- PM_data$new(\"src/your_Data.csv\") # replace your_Data.csv with your data file name",
      "# Option 2:",
      "dat <- PM_data$new(file.path(root, \"src/your_Data.csv\")) # replace your_Data.csv with your data file name",
      "",
      ...
      "# Option 2:",
      "run1 <- mod1$fit(data = dat, path = file.path(root, \"Runs\"))",
      "",
      "# run1 is in memory after the above completes, but to load the results if returning to the script later...",
      "# it is convenient to repeat the working directory here to avoid a lot of scrolling",
      "# Option 1:",
      glue::glue("setwd(\"{newFolder}\")"),
      "run1 <- PM_load(1, path = \"Runs\")",
      "# Option 2:",
      glue::glue("root <- \"{newFolder}\""),
      "run1 <- PM_load(1, path = file.path(root, \"Runs\"))",
      "",
      "# Sample Plots",
