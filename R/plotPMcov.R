...
plot.PM_cov <- function(x, 
  ...
  x <- if(inherits(x, "PM_cov")) {x$data} else {x}
  #include/exclude
  ...
  if(!timearg){ #if time is not part of the formula, collapse to summary
    x <- Pmetrics:::summary.PMcov(x, icen = icen) %>%
      Pmetrics:::includeExclude(include,exclude) %>%
  ...
  #defaults 
  if(is.null(line$lm)) {line$lm <- FALSE}
  if(is.null(line$loess)) {line$loess <- TRUE}
  if(is.null(line$ref)) {line$ref <- FALSE} 
  
  marker <- Pmetrics:::amendMarker(marker, default = list(color = "orange"))
  lmLine <- Pmetrics:::amendLine(line$lm, default = list(color = "dodgerblue", dash = "solid"))
  loessLine <- Pmetrics:::amendLine(line$loess, default = list(color = "dodgerblue", dash = "dash"))
  refLine <- Pmetrics:::amendLine(line$ref, default = list(color = "grey", dash = "dot"))
  ...
  #process dots
  layout <- Pmetrics:::amendDots(list(...))
  
  #legend - not needed for this function
  layout <- modifyList(layout, list(showlegend = F))
  
  #grid
  layout$xaxis <- Pmetrics:::setGrid(layout$xaxis, grid)
  layout$yaxis <- Pmetrics:::setGrid(layout$yaxis, grid)
  ...
  layout$title <- Pmetrics:::amendTitle(title, default = list(size = 20))
  
  #axis labels
  xlab <- if(missing(xlab)){vars[2]} else {xlab}
  ylab <- if(missing(ylab)){vars[1]} else {ylab}
  
  layout$xaxis$title <- Pmetrics:::amendTitle(xlab)
  if(is.character(ylab)){
    layout$yaxis$title <- Pmetrics:::amendTitle(ylab, layout$xaxis$title$font)
  } else {
    layout$yaxis$title <- Pmetrics:::amendTitle(ylab)
  }
