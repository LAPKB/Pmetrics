...
NPparse <- function(outfile = "NP_RF0001.TXT") {
  ...
  if (vernum < 3) {
    ...
    if (nauc > 0) {
      aucM <- matrix(scan(outfile, quiet = T, skip = 16 + 3 * nsub, n = nauc * 3), nrow = nauc, ncol = 3, byrow = T)
    } else {
      aucM <- NA
    }
    ...
    if (icyctot > 0) {
      ...
      if (nsub > 1) {
        ...
        isd <- matrix(NA, nrow = icyctot, ncol = nvar)
      }
      isd[isd == 0] <- NA
    ...
    sdata <- data.frame(
      id = checkID(sdata[, 2]), nsub = 1:nsub, age = as.numeric(sdata[, 3]),
    ...
    if (nofix == 0) {
      cat("There were no fixed parameters.\n")
    } else {
      cat("Fixed parameters:", parfix, "\n")
    }
    cat("Number of analyzed subjects:", nsub, "\n")
    cat("Number of output equations:", numeqt, "\n")
    cat("Additional covariates:", covnames, "\n")
    if (negflag) {
      cat("WARNING: There were negative or non-real pop/post predictions.\n")
    }
    coninterp <- switch(1 + converge,
      "The run did not converge before the last cycle.",
      "The run converged.",
      "",
      "WARNING: The run ended with a Hessian Error."
    )
    cat(coninterp, "\n")
    outlist <- list(
      nsub = nsub, nactve = nactve, nvar = nvar, nofix = nofix, par = par, parfix = parfix, covnames = covnames, ab = ab, valfix = valfix, ndim = ndim,
  ...
  if (vernum == 3) {
    ...
    # matrix with subj num, num of max times, and num of periods+1 for AUC tables
    if (nauc > 0) {
      aucM <- matrix(scan(outfile, quiet = T, skip = 23 + 3 * nsub, n = nauc * 3), nrow = nauc, ncol = 3, byrow = T)
    } else {
      aucM <- NA
    }
  ...
  if (vernum == 4) {
    ...
    # matrix with subj num, num of max times, and num of periods+1 for AUC tables
    if (nauc > 0) {
      aucM <- matrix(scan(outfile, quiet = T, skip = 22 + ndrug + 3 * nsub, n = nauc * 3), nrow = nauc, ncol = 3, byrow = T)
    } else {
      aucM <- NA
    }
  ...
  if (vernum == 5) {
    ...
    # matrix with subj num, num of max times, and num of periods+1 for AUC tables
    if (nauc > 0) {
      aucM <- matrix(scan(outfile, quiet = T, skip = 22 + ndrug + npost + 3 * nsub, n = nauc * 3), nrow = nauc, ncol = 3, byrow = T)
    } else {
      aucM <- NA
    }
    ...
    postden <- array(
      dim = c(npost, max(nactvepost[1:npost]), nvar + 1),
      dimnames = list(
        subj = 1:npost,
  ...
  if (vernum == 6) {
    ...
    if (icyctot > 0) {
      ...
      iaddl <- array(data = as.numeric(sub("D", "E", scan(outfile, skip = toc[18], n = icyctot * nvar * 12, quiet = T, what = ""))), dim = c(12, nvar, icyctot))
      dimnames(iaddl) <- list(
        info = c("mode", "skew", "kurt", "2.5%", "25%", "50%", "75%", "97.5%", "SD_IQR", "SD_95", "meanSD_IQR_95", "%scaledInfo"),
  ...
  if (vernum == 7) {
    ...
    cat("Random parameters:", par, "\n")
    if (nranfix == 0) {
      cat("There were no fixed but unknown parameters.\n")
    } else {
      cat("Fixed unknown parameters:", parranfix, "\n")
    }
    if (nofix == 0) {
      cat("There were no constant parameters.\n")
    } else {
      cat("Constant parameters:", parfix, "\n")
    }
    cat("Number of analyzed subjects:", nsub, "\n")
    cat("Number of output equations:", numeqt, "\n")
    cat("Additional covariates:", covnames, "\n")
    ...
    outlist <- list(
      nsub = nsub, nactve = nactve, nvar = nvar, nranfix = nranfix, nofix = nofix, par = par, parranfix = parranfix, parfix = parfix, covnames = covnames, ab = ab, valranfix = valranfix, valfix = valfix, ndim = ndim,
      indpts = indpts, icycst = icycst, icycmax = icycmax, icyctot = icyctot, converge = converge, ODEtol = ODEtol, prior = prior,
      ERRmod = ERRmod, numeqt = numeqt, ndrug = ndrug, salt = salt, ndose = ndose, ncov = ncov,
      nobs = nobs, nobsmax = nobsmax, numt = numt, corden = corden, postden = postden, pyjgx = pyjgx, ypredpop = ypredpop,
      ypredbay = ypredbay, ttpred = ttpred, exx = exx, ypredpopt = ypredpopt, ilog = ilog,
