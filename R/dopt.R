...
Dopt <- function(run, data, clean = T) {
  ...
  if (!missing(run)) {
    if (!file.exists(as.character(run))) {
      stop(paste(run, " not found in the current working directory.\n", sep = ""))
    }
    ...
    if (length(outfile) == 0) {
      ...
      if (tolower(outfile) == "end") {
        stop(paste("No output file specified.\n"))
      }
    }
    if (length(outfile) > 1) {
      ...
      if (tolower(outfileIndex) == "end") {
        stop(paste("No output file specified.\n"))
      } else {
        outfile <- outfile[as.numeric(outfileIndex)]
      }
    }
    ...
    if (!missing(data)) {
      if (file.exists(data)) {
        ...
        stop(paste(data, "is not in current directory.\n"))
      }
    } else {
    ...
    stop("Please supply a run number.\n")
  }
  ...
  if (data == "PATQZPX.001") {
    ControlFile <- c(
      outfile, # output file name file
      "1", # don't change density file
      "1", # don't change  data file
      "1"
    ) # don't change model file
  } else {
  ...
  if (OS == 1 | OS == 3) {
    system("./DOprep.exe MacOSX < DOcontrol", ignore.stdout = T)
  }
  if (OS == 2) {
    shell("DOprep.exe DOS < DOcontrol")
  }
