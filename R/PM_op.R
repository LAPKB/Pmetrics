  ...
  make = function(data, path) {
    if (file.exists(file.path(path, "op.csv"))) {
      ...
      cli::cli_warn(c(
        ...
        "i" = "Result does not have valid {.code PM_op} object, and {.file {file.path(path, 'op.csv')} does not exist."
      ))
      return(NULL)
    }
    ...
    poly <- map(config$errormodels$models, \(x) x %>% pluck("poly") %>% unlist()) %>% bind_rows()
    op <- op_raw %>%
