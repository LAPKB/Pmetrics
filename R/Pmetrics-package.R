...
#' @keywords internal
#'
#' @importFrom dplyr select arrange filter mutate group_by row_number group_map ungroup bind_cols bind_rows nest_by rowwise relocate rename reframe inner_join slice_tail slice_head slice across tibble as_tibble quo pull n everything case_match case_when starts_with all_of where distinct summarize if_else
...
#' @importFrom graphics abline arrows axTicks axis boxplot hist legend lines par plot points polygon rect rug segments text
