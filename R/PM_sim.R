...
PM_sim <- R6::R6Class(
  ...
  public = list(
    ...
    #' "c_model.txt").
    #'
    #' @param usePost Boolean argument.  Only applicable when `poppar` contains an
    ...
    SIMrun = function(poppar, limits, model, data, split,
      ...
      for (i in 1:nsub) {
        ...
        if (!is.na(predTimes[1])) {
          ...
          predTimes <- predTimes[!predTimes %in% temp$time[temp$evid == 0]] # remove prediction times at times that are specified in template
