...
makeErrorPoly <- function(obs, sd, data, outeq = 1, col = "red", cex = 3, pch = "+", lcol = "blue", lwd = 2, ref = T, legend = T, ...) {
  ...
  if (missing(data)) {
    ...
    maxobs <- max(data$out[data$evid == 0 & data$outeq == outeq], na.rm = T)
  }
  ...
  if (!bad.3) {
    r.3 <- round(summary(lm.3)$r.squared, 5)
  } else {
    r.3 <- "NA"
  }
  ...
  if (!bad.2) {
    predict.2 <- predict(lm.2, data.frame(obs = newConc))
  } else {
    predict.2 <- predict.1
  }
  if (!bad.3) {
    predict.3 <- predict(lm.3, data.frame(obs = newConc))
  } else {
    predict.3 <- predict.1
  }
  ...
  if (length(arglist) > 0) {
    if (!"ylim" %in% names(arglist)) {
      arglist <- list(arglist, ylim)
    }
  } else {
    arglist <- list(ylim = ylim)
  }
  par(mar = c(5, 5, 4, 2) + 0.1)
  do.call(plot, args = c(list(
    y = sd, x = obs, type = "n", xlim = base::range(newConc),
  ...
  if (legend) {
    legend(
      x = "topleft", legend = c(
        as.expression(substitute(paste(1^{
          st
        }, " degree ", R^2, " = ", r.1), list(r.1 = r.1))),
        as.expression(substitute(paste(2^{
          nd
        }, " degree ", R^2, " = ", r.2), list(r.2 = r.2))),
        as.expression(substitute(paste(3^{
          rd
        }, " degree ", R^2, " = ", r.3), list(r.3 = r.3)))
      ),
