...
NM2PM <- function(data, ctl) {
  ...
  while (!file.exists(data)) {
    ...
    if (tolower(data) == "end") {
      msg <- c(msg, "No data file specified.\n")
      cat(msg)
  ...
  while (!file.exists(ctl)) {
    ...
    if (tolower(ctl) == "end") {
      msg <- c(msg, "No control file specified.\n")
      cat(msg)
      return(invisible(NULL))
    }
  }
  ...
  if (length(inputLine) > 0) {
    ...
    stop(paste("No \\$INPUT line found in ", ctl, "\n", sep = ""))
  }
  ...
  if (is.na(colNums$AMT)) {
    stop("There does not appear to be an AMT column in the data.\n")
  } else {
    dataDF[, colNums$AMT] <- as.numeric(dataDF[, colNums$AMT])
  }
  if (is.na(colNums$DV)) {
    stop("There does not appear to be a DV column in the data.\n")
  } else {
    dataDF[, colNums$DV] <- as.numeric(dataDF[, colNums$DV])
  }
  ...
  if (is.na(colNums$EVID)) {
    ...
    dataDF[, colNums$EVID] <- as.numeric(dataDF[, colNums$EVID])
  }
  # CMT is recoverable
  if (is.na(colNums$CMT)) {
    ...
    dataDF[, colNums$CMT] <- as.numeric(dataDF[, colNums$CMT])
  }
  # RATE is recoverable
  if (is.na(colNums$RATE)) {
    ...
    dataDF[, colNums$RATE] <- as.numeric(dataDF[, colNums$RATE])
  }
  ...
  if (!is.na(TIMEcol)) {
    ...
    if (length(colons) > 0) { # clock format times present
      if (is.null(dateFormat)) { # no date so convert any hh:mm to h.h
        ...
        if (length(colons) < nrow(dataDF)) {
          dataDF[-colons, TIMEcol] <- sapply(dataDF[-colons, TIMEcol], decimal2Clock)
        }
        clockTime <- T
        timeFormat <- "h:m"
      }
  ...
  if (clockTime) {
    relTime <- PMmatrixRelTime(dataDF,
      idCol = colNums$ID, dateCol = DATEcol, timeCol = TIMEcol, evidCol = colNums$EVID,
