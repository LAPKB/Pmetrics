  ...
  server <- function(input, output, session) {
              ...
              accordion_panel(
                "Line Options",
                  ...
                  bslib::nav_panel(
                    ...
                    checkboxInput("def_sim_fmt","Use default formatting", TRUE),
                    conditionalPanel(
                      condition = "!input.def_sim_fmt",
                      selectInput("sim_probs","Quantiles",
                                  choices=c(0.01,0.025,0.05,0.10,0.25,0.50,0.75,0.90,0.95,0.975,0.99),
    ...
    makePMplot <- function(code = FALSE){
      ...
      if(!is.null(input$data) && 
        ...
        p <- suppressWarnings(do.call(plot, args))
        if(code){
          return(codeStatement)
        } else {
          return(p)
