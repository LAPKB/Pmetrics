...
PMFortranConfig <- function(reconfig = F) {
  ...
  if (file.exists(configFilename)) {
    ...
    if (getBits() == "64" & length(compiler) == 1 & length(grep("^gfortran", compiler) > 0)) {
      compiler <- c(compiler, "gfortran -march=native -o <exec> <files>")
    ...
    while (is.na(choice)) {
      ...
      if (as.numeric(choice) < 5) {
        compiler <- switch(as.numeric(choice),
          sprintf("%s -march=native -w -O3 -o <exec> <files>\n%s -march=native -w -fopenmp -fmax-stack-var-size=32768 -O3 -o <exec> <files>", gfortran_command, gfortran_command),
