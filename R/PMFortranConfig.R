...
PMFortranConfig <- function(reconfig = F) {
  ...
  if (file.exists(configFilename)) {
    ...
    if (getBits() == "64" & length(compiler) == 1 & length(grep("^gfortran", compiler) > 0)) {
      compiler <- c(compiler, "gfortran -Ofast -march=native -w -fopenmp -fmax-stack-var-size=32768 -o <exec> <files>")
  ...
  if (is.na(compiler[1]) | reconfig) {
    ...
    cat(paste("1. gfortran -m", getBits(), " -Ofast -march=native -w -o <exec> <files>\ngfortran -Ofast -march=native -w -fopenmp -fmax-stack-var-size=32768 -o <exec> <files>\n", sep = ""))
    ...
    while (is.na(choice)) {
      ...
      if (as.numeric(choice) < 5) {
        compiler <- switch(as.numeric(choice),
                           paste("gfortran -m", getBits(), " -Ofast -march=native -w -o <exec> <files>\ngfortran -Ofast -march=native -w -fopenmp -fmax-stack-var-size=32768 -o <exec> <files>", sep = ""),
        ...
        if (choice == "5") {
          ...
          cat("\nExample: gfortran -Ofast -o <exec> <files>\n")
