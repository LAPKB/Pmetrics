...
notNeeded <- function(x, f) {
  cli::cli_inform(c(
    "i" = "{.arg x} is not needed for {.fn f}.",
...
add_smooth <- function(
  ...
  if (identical(method, "lm")) {
    ...
    p_data <- if (!is.null(data)) {
      data
    } else {
      plotly::plotly_data(p)
    }
...
rgba_to_rgb <- function(rgba_str, alpha = NULL) {
  ...
  if (is.na(rgba_str) | stringr::str_detect(rgba_str, "#")) {
    return(rgba_str)
  }
...
plotlygg <- function(p, print = TRUE) {
  if (inherits(p, "trelliscopejs_widget")) {
    cli::cli_abort(c(
      "x" = "Plotting data with {.arg overlay = TRUE} results in a {.cls treliscopejs_widget} object, which cannot be converted to ggplot.",
  ...
  bar_position <- switch(lay$barmode %||% "",
    ...
    "relative" = "stack",
    "overlay"
  ) # ggplot's default is "stack"; "overlay" ~ "identity"
