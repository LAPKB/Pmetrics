  ...
  if (missing(formula)) { # univariate plot
    ...
    if (inherits(data, "NPAG")) {
      ...
      for (i in 1:(ncol(data$popPoints) - 1)) {
        if (!add) {
          plot(data$popPoints[, "prob"] ~ data$popPoints[, i],
            type = "h", lwd = lwd, col = col, xlab = names(data$popPoints)[i], xlim = data$ab[i, ],
    ...
    if (inherits(data, "IT2B")) {
      if (yCol == "prob") { # posterior plot
        ...
        for (j in 1:nsub) {
          plot(this.y[j, ] ~ this.x,
            type = "l", lwd = lwd, col = col, ylab = "Probability",
            xlab = attr(terms(formula), "term.labels"), cex.lab = cex.lab,
            main = paste("Subject", data$postMean$id[j]), xlim = xlim, ylim = ylim, ...
          )
          if (ref) {
            lines(y.pop ~ x.pop, type = "l", col = col.ref.trans, lwd = lwd.ref)
          }
        }
      } else {
        ...
        if (!missing(legend)) {
          if (inherits(legend, "list")) {
            ...
            if (legend) {
              legend <- {
                list(plot = T, x = "topright", legend = 1 - probs, fill = graycols, title = "Quantile")
              }
            } else {
    ...
    if (inherits(data, "NPAG")) {
      if (yCol == "prob") { # posterior plot
        ...
        for (i in 1:nsub) {
          ...
          plot(y ~ x,
            type = "h", lwd = lwd, col = col, xlim = xlim, ylim = ylim, ylab = "Probability",
            xlab = attr(terms(formula), "term.labels"), cex.lab = cex.lab, main = paste("Subject", subjID[i]), ...
          )
          if (ref) {
            lines(y.pop ~ x.pop, type = "h", col = col.ref.trans, lwd = lwd.ref)
          }
        }
