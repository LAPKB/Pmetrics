...
#' @param x The name of an *PM_final* data object generated by [makeFinal]
...
#' data(final.2)
#' plot(final.2)
plot.PM_final <- function(x, formula, include, exclude, xlab, ylab, line=T, density=F, grid=T){
  ...
  uniPlot <- function(param, name, min, max, type){
    ...
    p <- p %>% add_annotations(
      ...
      layout(showlegend = F, xaxis = list(range = c(min, max)), shapes = list(vline(min, width=3, dash="dash"), vline(max, width=3, dash="dash")))
    return(p)
  }
