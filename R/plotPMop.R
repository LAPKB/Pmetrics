...
#' as `marker = list(color = orange, shape = "circle", size = 10, opacity = 0.5, line = list(color = black, width = 1))`.
#' Defaults for regression and reference lines are detailed in the `line` argument
#' below. 
#' @method plot PMop
#' @param x The name of an *PM_op* or *PMop* data object generated by [makeOP].
...
#' @template xlim 
#' @template ylim 
#' @param xlab Label for the x-axis.  If missing, will default to "Predicted" for 
#' plots when `resid = F` and either "Time" or "Predicted" for residual plots.
#' @param ylab Label for the y-axis.  If missing, will default to "Observed" for
...
plot.PM_op <- function(x, 
  ...
  if(!resid){
    ...
    #axis labels
    xlab <- if(missing(xlab)){"Predicted"}
    ylab <- if(missing(ylab)){"Observed"}
    
    xaxis <- amendAxisLabel(xaxis, xlab)
    yaxis <- amendAxisLabel(yaxis, ylab)
    
    #log axes
    if(log){
      xaxis <- modifyList(xaxis, list(type = "log"))
    ...
    if(max(x$obs, na.rm = T) > max(x$pred, na.rm = T)){ #anchor is y axis
      xaxis <- modifyList(xaxis, list(matches = "y"))
    } else { #anchor is x axis
      yaxis <- modifyList(yaxis, list(matches = "x"))
    }
    
    #finalize layout
    layoutList <- modifyList(layout, list(xaxis = xaxis, yaxis = yaxis, shape = NULL))
    ...
    if(refLine$plot){
      ...
      refLine$plot <- NULL
      layoutList$refLine <- list(type = "line",
                                 x0 = 0,
                                 y0 = 0,
                                 x1 = 1,
                                 y1 = 1,
    ...
    #set layout
    xaxis <- amendAxisLabel(xaxis, "Time")
    ...
    #set layout
    layoutList$xaxis <- amendAxisLabel(xaxis, "Predicted")
    p2 <- p2 %>%
      plotly::layout(xaxis = layoutList$xaxis,
                     yaxis = layoutList$yaxis,
                     showlegend = layoutList$showlegend)
