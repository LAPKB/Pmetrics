...
plot.PM_sim <- function(x, 
  ...
  if(is.logical(line)){
    ...
    if(!is.list(line) & length(line)>1) {
      stop(paste0("Specify the line argument as ",
  ...
  probFormats <- lineList[-1] %>% purrr::transpose() %>% purrr::simplify_all()
  ...
  if(!all(is.na(probValues)) & nsim>=10){
    ...
    sim_quant_df <- sim %>% 
      ...
      dplyr::mutate(time = rep(times, each = length(probValues)),
...
#' replaced by [plot.PM_sim]. Simulated observations are plotted as quantiles on the y-axis vs. time on the x.axis.  If measured
#' observations are included, a visual and numerical predictive check will be performed.
#'
#' @method plot PMsim
