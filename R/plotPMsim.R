...
#' @param ci Width of confidence interval bands around simulated quantiles, from 0 to 1.  If 0, or *nsim*<100, will not plot.
...
#' `marker = list(color = "red", symbol = "triangle", size = 10`). Use the plotly `plotly::schema()` command
...
#' @param grid If `FALSE`, the default, no grid plotted. If `TRUE`, the default 
#' color *grey50* and width 1 will be plotted at major tick marks. If a list,
#' color and width can be customized, e.g. `grid = list(gridcolor = "black", gridwidth = 2)`.
#' The grid parameter modifies the xaxis and yaxis layout object in plotly. 
#' See `plotly::schema()` layout > xaxis/yaxis > gridcolor/gridwidth.
#' @param xlab Label for x-axis; default is "Time". The xlab parameter modifies 
#' the xaxis title in the layout object in plotly. 
#' See `plotly::schema()` layout > xaxis > title.
#' @param ylab Label for y-axis; default is "Output". The ylab parameter modifies 
#' the yaxis title in the layout object in plotly. 
#' See `plotly::schema()` layout > yaxis > title.
#' @param xlim Limits of the x-axis as a vector, e.g. `c(0,1)`.  
#' The xlim parameter modifies 
#' the xaxis range in the layout object in plotly. 
#' See `plotly::schema()` layout > xaxis > range
#' @param ylim Analogous to `xlim` for the yaxis.
#' @param legend Either a boolean operator (most common) or a list of parameters to be supplied to plotly.
#' If `FALSE` (the default), a legend will not be plotted. If `TRUE` the plotted quantiles
#' are the legend. See `plotly::schema()` > layout > layoutAttributes > legend
#' for more details on the available options for formatting. For example, 
#' `legend = list(orientation = "h", font = list(color = "blue"))`. Note that if a
#' legend is included, clicking on a quantile item in the legend will hide it in the plot,
...
#' the folowing items:
#' \item{npc}{A dataframe with three columns: quantile, prop_less, pval.  *quantile* are those specified
#' by the `probs` argument to the plot call; *prop_less* are the proportion of simulated
#' observations at all times less than the quantile; *pval* is the P-value of the difference in the 
...
#' \item{obs}{A data frame similar to an PMop object made by [makeOP] at the end of a run
