...
#' NPex$cycle$plot(linetypes = "dash", colors = "Blues", marker = list(size = 1), 
#' line = list(width = 3))
#' NPex$cycle$plot(grid = F, 
...
plot.PM_cycle <- function(x, 
  ...
  xlab <- if(missing(xlab)){"Cycle Number"} else {modifyList(xlab,list(text="Cycle Number"))}
  ylab <- if(missing(ylab)){""} else {modifyList(ylab,list(text=""))}
  ...
  p2 <- graph_data  %>%
    plotly::plot_ly(x = ~cyc, y = ~aic, type = "scatter", mode = "lines+markers",
                    name = "AIC",
                    line = line, 
                    marker = marker, 
                    text = ~bic,
  ...
  p3 <- graph_data  %>%
    plotly::plot_ly(x = ~cyc, y = ~gamlam, type = "scatter", mode = "lines+markers",
                    line = line,
                    marker = marker,
  ...
  normalized_plot <- function(.par, range){
    ...
    .p <- graph_data[[.par]] %>% bind_cols(cyc=graph_data$cyc) %>% pivot_longer(cols = -cyc, names_to = "par") %>%
      plot_ly(x = ~cyc, y = ~value, type = "scatter", mode = "markers+lines", 
              color = ~par, 
              colors = colors,
