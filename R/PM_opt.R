#' @title MM-optimal Sample Times
#' @description
#' `r lifecycle::badge("stable")`
#'
#' Method for [PM_result] objects to computes 1 or more MM-optimal sampling times.
#' @details
#' Based on the multiple-model optimization algorithm, this object contains
#' the methods and results from the computation of the requested number
#' of sample times where the concentration time profiles are the most separated, 
#' thereby minimizing the risk of choosing the incorrect 
#' Bayesian posterior for an individual. The algorithm was published as 
...
PM_opt <- R6::R6Class(
  "PM_opt",
  public <- list(
    #' @field sampleTime The MM-optimal sample times
    sampleTime = NULL,
    #' @field bayesRisk The Bayesian risk of mis-classifying a subject based on the sample times.  This is more useful for comparisons between sampling strategies, with minimization the goal.
    bayesRisk = NULL,
    #' @field simdata A [PM_sim] object with the simulated profiles
    simdata = NULL,
    #' @field mmInt A list with the `mmInt` values, `NULL` if `mmInt` is missing.
    mmInt = NULL,
    #' @description
    #' Create new object 
    #' @details
    #' Creation of new `PM_opt` objects is via the `mmopt` method for [PM_result] objects.
    #' @param ... Parameters passed to the `$make` method.
    initialize = function(...) {
      tryCatch(self$make(...), error = function(e){
    ...
    #' @param ... Other parameters to pass to [SIMrun]. Most are not necessary,
    ...
    make = function(poppar, model, data, nsamp = 1, weight = list(none = 1),
      ...
      #TODO: change so that other poppar can be used, e.g. posteriors
