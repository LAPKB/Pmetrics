...
PM_opt <- R6::R6Class(
  ...
  public = list(
    ...
    #' @param model A [PM_model] object, required or optional depending on the `poppar` argument. 
    ...
    #' @param data A [PM_data] object, required or optional depending on the `poppar` argument.
    ...
    outeq = 1, ...) {
      if (missing(poppar)) {
        cli::cli_abort(c("x" = "{.arg poppar} is required."))
        return(NULL)
      }
      if (missing(model)) {
        model <- NULL
  ...
  private = list(
    ...
    algorithm = "mm", ...) {
      ...
      # CASE 1 - PM_sim, simulation already done
      if (inherits(poppar, "PM_sim")){
        case <- 1
        simdata <- poppar
        popPoints <- simdata$data$parValues %>% select(-nsim) %>% 
      ...
      } else if (inherits(poppar, "PM_sim_data")) {
        case <- 2
        ...
        # CASE 3 - PM_result, will need to simulate
      } else if (inherits(poppar, "PM_result")) {
        case <- 3
        if (!inherits(poppar$final, "NPAG")) {
          cli::cli_abort(c("x" = "Prior run must be NPAG."))
          return(NULL)
        }
        ...
        model <- if (is.null(model)) {
          cli::cli_abort(c("x" = "When {.arg poppar} is a [PM_final] object, {.arg model} must be specified."))
        } else {
          model
        }
        data <- if (is.null(data)) {
          cli::cli_abort(c("x" = "When {.arg poppar} is a [PM_final] object, {.arg data} must be specified."))
        } else {
          data
        }
        
        # CASE 5, data frame (i.e. theta), need to simulate
      } else if (is.data.frame(poppar)){
        case <- 5
        
        model <- if (is.null(model)) {
          cli::cli_abort(c("x" = "When {.arg poppar} is a data frame of support points, {.arg model} must be specified."))
        } else {
          model
        }
        data <- if (is.null(data)) {
          cli::cli_abort(c("x" = "When {.arg poppar} is a data frame of support points, {.arg data} must be specified."))
        } else {
          data
        }
        

        # CASE 6, list of weights, means, and covariance, need to simulate
      } else if (is.list(poppar)) { # poppar was some other list and simulator will error trap
        case <- 6
        
        model <- if (is.null(model)) {
          cli::cli_abort(c("x" = "When {.arg poppar} is a list, {.arg model} must be specified."))
        } else {
          model
        }
        data <- if (is.null(data)) {
          cli::cli_abort(c("x" = "When {.arg poppar} is a list, {.arg data} must be specified."))
        } else {
          data
        }
      } else {
        cli::cli_abort(c("x" = "{.arg poppar} is not a valid object."))
      }
...
wmmopt1 = function(Mu, time, pH, cassay, nsamp, nsubs, nout, Cbar) {
  ...
  search_grid <- utils::combn(1:nout, nsamp)
  ncomb <- ncol(search_grid)
  cli::cli_text("Calculating optimal times for {nsamp} sample points...")
  pb <- progress::progress_bar$new(total = ncomb, clear = FALSE, force = TRUE, show_after = 0)
  on.exit(pb$terminate(), add = TRUE)

  # Use parallel workers for large search grids when mc.cores > 1.
  mc_cores <- getOption("mc.cores", 1L)
  use_parallel <- mc_cores > 1L && ncomb > 1000L

  if (use_parallel) {
    n_workers <- max(1L, min(as.integer(mc_cores), ncomb))
    cl <- parallel::makeCluster(n_workers, type = "PSOCK")
    on.exit(parallel::stopCluster(cl), add = TRUE)

    parallel::clusterExport(
      cl,
      varlist = c("pH", "Kall", "Cbar", "search_grid"),
      envir = environment()
    )

    idx <- seq_len(ncomb)
    chunk_size <- max(100L, n_workers * 10L)
    chunk_id <- ceiling(seq_along(idx) / chunk_size)
    idx_chunks <- split(idx, chunk_id)
    Perror <- numeric(ncomb)

    for (chunk in idx_chunks) {
      chunk_vals <- unlist(parallel::parLapply(cl, chunk, function(i) {
        nvec <- search_grid[, i]
        nsubs <- dim(Kall)[1]
        nsamp <- length(nvec)
        pH5 <- sqrt(pH)
        ...
        as.numeric(.5 * t(pH5) %*% (ExpKallsum0 * Cbar) %*% pH5)
      }), use.names = FALSE)

      Perror[chunk] <- chunk_vals
      pb$tick(length(chunk))
    }
  } else {
    Perror <- vapply(
      seq_len(ncomb),
      function(i) private$perrorc1(pH, Kall, nvec = search_grid[, i], Cbar, pb = pb),
      numeric(1)
    )
  }

  min_idx <- which.min(Perror)
  nopt <- sort(search_grid[, min_idx])
  ...
  brisk_cob[nsamp] <- Perror[min_idx]
...
plot.PM_opt <- function(x, line = list(probs = NA), times = T, print = TRUE, ...) {
  ...
  p <- do.call(plot.PM_sim, c(list(x = x$simdata), arglist))$p
