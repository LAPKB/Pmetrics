...
C  ONE POTENTIALLY FOR EACH BOLUS INPUT OF UP TO NTAU DRUGS.
        ...
        double precision, dimension(5000), intent(INOUT) :: SIG
        integer, intent(IN) :: NDRUG, NADD
        double precision, dimension(5000,34), intent(INOUT) :: RS
        integer, dimension(128), intent(INOUT) :: INTLIST

        integer, dimension(7) :: INDIV, INDBOL
        integer, dimension(20) :: INDCOV
        double precision, dimension(34) :: TIMCAN

        integer I,IDOSE,IND,ISAME1,ISAME2,ISAME3,NI
        double precision RR,TIMNXT,VALAST

! NEW PARALLEL CODE BELOW AS OF npageng28.f.
        double precision, dimension(7,5000,2), save :: XIV, BOL
        double precision, dimension(20,5000,2), save :: COV
...
C wmy2018.07.20 -- Changed tau(7) to tau(:)
        if (I.ne.7) then
          write (*,*) "ERROR: In SR SHIFT(), size(tlag).ne.7"
        endif
