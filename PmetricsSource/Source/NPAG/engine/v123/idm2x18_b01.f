...
c  idcy_53e is part of the big npem program, npbig4.f.
	...
	SUBROUTINE IDCALCY(JSUB,IG,NPP,NDIM,ESTML,YPRED,NUMEQT,NOBSER,
     1    MF,RTOL,ATOL,RSCOPY,BSCOPY,INTLIST,IPAR,ObsError,RPAR)
...
C        IMPLICIT REAL*8(A-H,O-Z)
        ...
        double precision, dimension(5000,34) :: RSCOPY
        double precision, dimension(5000,7) :: BSCOPY
...
C        double precision, dimension(32) :: P
...
c
        CALL FUNC2(JSUB,IG,NOBSER,YPRED,NUMEQT,
     1      NPP,ESTML,NDIM,MF,RTOL,ATOL,INTLIST,IPAR,ObsError,RPAR)

        write (*,*) "Returning to NPAG"
...
C
	SUBROUTINE FUNC2(JSUB,IG,M,YPRED,NUMEQT,
     1      NPP,ESTML,NDIM,MF,RTOL,ATOL,INTLIST,IPAR,ObsError,RPAR)
...
!$omp Threadprivate(/CNST/,/CNST2/)
!$omp Threadprivate(/PARAMD/,/INPUT/)

      integer, dimension(7) :: NBCOMP
      double precision, dimension(594) :: TIM
      double precision, dimension(5000) :: SIG
      double precision, dimension(5000,34) :: RS
      double precision, dimension(594,MAXNUMEQ) :: YO
...
C      DIMENSION X(20),P(32),TIM(594),SIG(5000),SIGO(5000),R(37),
C     1 RS(5000,34),RSO(5000,34),YT(MAXNUMEQ),YO(594,MAXNUMEQ),
C     2 YPRED(594,NUMEQT), BS(5000,7),Y(594,MAXNUMEQ),B(20),NBCOMP(7),
C     3 TLAG(7),FA(7),XSTORE(100,20),XPRED(20),XVERIFY(100)

C------ Argument List
      integer JSUB,IG,M,NPP,NDIM,MF,NUMEQT
      double precision RTOL
      double precision, dimension(594,NUMEQT) :: YPRED
      integer, dimension(128) :: INTLIST
      integer, dimension(257) :: IPAR
      real*8, dimension(20) :: ATOL
      real*8, dimension(32) :: ESTML
      double precision, dimension(594,MAXNUMEQ) :: ObsError
      double precision, dimension(257) :: RPAR
...
c  (apply SAVE attribute to params that are passed to subroutines)
      ...
      double precision, save, dimension(20) :: X, XPRED
      double precision, save, dimension(100) :: XVERIFY
      double precision, save, dimension(100,20) :: XSTORE
      double precision, save, dimension(5000) :: SIGO
      double precision, save, dimension(5000,34) :: RSO
      double precision, save, dimension(7) :: TLAG, FA
...
C*****DETERMINE YPRED(I)*****
        ...
        write (*,*) "Writing YPRED(1:#meas,1:#out_eq)",M,NOS
...
CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
C
C	SUBROUTINE EVAL2(YPRED,NUMEQT)
      SUBROUTINE EVAL2(JSUB,IG,NPP,ESTML,YPRED,NUMEQT,NOBSER,NDIM,MF,
     1      RTOL,ATOL,RSCOPY,BSCOPY,INTLIST,IPAR,ObsError)
...
C wmy2017Sep13 Not sure why /SUM2/ and /CNST2/ are declared here
C   as they are not used here.
C      COMMON/SUM2/ M,NPNL
C      COMMON/CNST2/ NPL,NOS,NDRUG,NADD

        integer JSUB, IG, NPP, NUMEQT, NOBSER, NDIM, MF

        real*8, DIMENSION(594,NUMEQT) :: YPRED
        integer, dimension(128) :: INTLIST
        integer, dimension(257) :: IPAR
        real*8, dimension(20) :: ATOL
        real*8, dimension(32) :: ESTML
...
c         write (*,*) "In EVAL2 :: Calling FUNC2(M,NOBSER,NUMEQT,NPP)",
c     1    M, NOBSER,NUMEQT,NPP

C        CALL FUNC2(M,YPRED,NUMEQT)
        CALL FUNC2(JSUB,IG,NOBSER,YPRED,NUMEQT,
     1      NPP,ESTML,NDIM,MF,RTOL,ATOL,INTLIST,IPAR,ObsError)

          write (*,*) "Returning to IDCALCY"

      RETURN
      END


C------ SUBROUTINE IDCALCY() on Jan 11, 2018
	SUBROUTINE old_IDCALCY(JSUB,IG,NPP,NDIM,ESTML,YPRED,NUMEQT
...
C wmy2017Sep14
        integer JSUB,IG,NOBSER, NDIM,MF
        real*8  RTOL
        real*8, dimension(20) :: ATOL
        real*8, dimension(5000,34) :: RSCOPY
...
C        write (*,*) "Initializing /PARAMD/P with ESTML", NP 
...
C  AT ESTML(I) AS DEFINED ABOVE.
        ...
        write (*,*) "/PARAMD/P initialized; Calling EVAL2"
        write (*,*) "(NPP,NOBSER,NDIM)",NPP,NOBSER,NDIM
        write (*,*) "BUG 9/18/2017 NOBSER=0, not 8 (Ashley's Greco)"


	CALL EVAL2(JSUB,IG,NPP,ESTML,YPRED,NUMEQT,
     1      NOBSER, NDIM,MF,RTOL,ATOL,INTLIST,IPAR,ObsError)
