...
C	CALL EVAL3(JSUB,IG,NPP,ESTML,NUMT,YYPRED,TPRED,NUMEQT,
C     1      NDIM,MF,RTOL,ATOL,INTLIST,IPAR,ObsError)

        CALL FUNC3(JSUB,IG,NUMT,YYPRED,TPRED,NUMEQT,
...
C
      SUBROUTINE FUNC3(JSUB,IG,NUMT,YYPRED,TPRED,NUMEQT,
     1      NPP,ESTML,NDIM,MF,RTOL,ATOL,INTLIST,IPAR,ObsError,RPAR)
...
! NEW PARALLEL CODE BELOW AS OF npageng28.f.
C !$omp Threadprivate(/PARAMD/,/INPUT/,/STATE/,/CNST/)
!$omp ThreadPrivate(/BOLUSCOMP/,/OBSER/)
!$omp Threadprivate(/PARAMD/,/INPUT/,/CNST/,/CNST2/)
...
c !$omp ThreadPrivate( IPAR, ObsError ) ! conflicts w/dummy attribute
...
CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
C
	SUBROUTINE EVAL3(JSUB,IG,NPP,ESTML,NUMT,YYPRED,TPRED,NUMEQT,
     1      NDIM,MF,RTOL,ATOL,INTLIST,IPAR,ObsError)
...
C       COMMON/CNST2/ NPL,NOS,NDRUG,NADD
        ...
        real*8, DIMENSION(71281,NUMEQT) ::YYPRED
        real*8, dimension(71281) :: TPRED

        integer JSUB, IG, NPP, NUMT, NUMEQT, NDIM, MF       
        integer, dimension(128) :: INTLIST
        integer, dimension(257) :: IPAR
        real*8, dimension(20) :: ATOL
        real*8, dimension(32) :: ESTML
...
C !$omp ThreadPrivate( IPAR, ObsError ) in main
