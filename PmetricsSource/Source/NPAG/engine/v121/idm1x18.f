...
C-----------------------------------------------------------------------
        ...
        DIMENSION X(32),P(32),R(37),B(20)
        COMMON/CNST/ N,ND,NI,NUP,NUIC,NP
        COMMON/PARAMD/ P
        COMMON/INPUT/ R,B

        integer NPX,NOBSER,NUMEQT,NDIM,MF
        real*8 RTOL, ATOL(20), RCOPY(37), BCOPY(20)
...
CCC HERE ... in above, parallel = F, works; but if R and B
C  are replaced by RCOPY and BCOPY -- major fail!!!

C wmy 2017Sep11 SUMSQ receives SUMSQJ, therefore no need for following line
C        SUMSQJ=SUMLM  


         write (*,*) "SUMSQ will return SUMSQJ =", SUMSQJ
...
!$omp Threadprivate(RSO,BSO,SIGO,Y)   
...
C   IF(TIM(KNT) .EQ. 0.D0 .AND. KNT .GT. 1)  CONDITION.
      ...
      TOUT=TIM(KNT)
      KNT=KNT+1
        write (*,*) "IF(TIM(KNT) .NE. SIGO(KNS)) KNT = ", KNT,JSUB,IG
...
30      CONTINUE
      ...
      write (*,*) JSUB,IG,"FUNC->USER ::",KNT,T,X(1),X(2),X(3)
      write (*,*) JSUB,IG,"FUNC->USER ::",R(1),R(3),R(4),R(5),R(6),R(7)
...
C        IF(N .EQ. -1) CALL ANAL3(X,T,TOUT)
      ...
      write (*,*) JSUB,IG,"USER->FUNC ::",KNT,T,X(1),X(2),X(3)
