...
c wmy2017Sep08
       ...
       real*8 W
       real*8, dimension(32) :: ParamArray
...
!$omp&FirstPrivate(JSUB,SIGFAC,OFAC,NOBSER,NUMEQT)
!$omp&FirstPrivate(IterFirst,IterLast,NSTORE,NACTVE,MAXACT)
!$omp&FirstPrivate(IRAN,NVAR,CORDEN,NOFIX,VALFIX,NRANFIX,RANFIXEST)
C !$omp&ThreadPrivate(/TOCALC/,W)
...
C   created w/random value, but is initialized 
...
C wmy2017Sep12 Added /TOUSER/ varbs to CALL IDPC()
        CALL IDPC(NPX,ParamArray,W,NOBSER,NUMEQT,
     1    NDIM,MF,RTOL,ATOL)
...
C        write (*,*) "(Start,IG,Stop) =",IterFirst,IG,IterLast
...
C  PARAMETER (THOSE WHICH ARE UNKNOWN BUT THE SAME FOR ALL SUBJECTS).
      ...
      CALL MAKEVEC(NVAR,NOFIX,NRANFIX,IRAN,EXXX,VALFIX,RANFIXEST,PX)
         ...
         write (*,*) JSUB,"Calling IDCALCY", NVAR+NOFIX+NRANFIX,NDIM
     1      ,PX(21),YPRED(1,2),NUMEQT,NOBSER,NDIM,MF,RTOL,ATOL,MF
...
C        CALL IDPC(NPX,PX,W,NOBSER,NUMEQT)
        CALL IDPC(NPX,PX,W,NOBSER,NUMEQT,
     1    NDIM,MF,RTOL,ATOL)
