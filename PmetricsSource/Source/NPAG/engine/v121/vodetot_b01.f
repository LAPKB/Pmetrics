...
C  REMAINS THE MAX. NO. OF OBSERVATION TIMES (AS OF NPAG113.FOR).
     ...
     2 VALFIX(20),CENTER(3,30),EXXX(30),IRAN(32),PX(32),
...
C declared in one of the blocks above: IRAN(32)

c wmy2017Sep08 DO 800 variables
       integer IterFirst, IterLast, NPX
       real*8 W, XXIG(30)
...
C wmy2017Sep26 Copies of /INPUT/, /PARAMD/
       real*8, dimension(32) :: P, PCOPY
       real*8, dimension(37) :: R, RCOPY
       real*8, dimension(20) :: B, BCOPY
       real*8, save, dimension(5000,34) :: RSCOPY
       real*8, save, dimension(5000,7) :: BSCOPY
...
C  SUBROUTINES.
	...
	CALL FILRED(NOBSER,YO,C0,C1,C2,C3,NUMEQT,RSCOPY,BSCOPY,
     1    INTLIST)
...
C
      ...
      write (*,*) "First and Last Support are" , IterFirst, IterLast
      write (*,*) "Gamma = ", gamma
C
...
! NEW PARALLEL CODE BELOW AS OF npageng28.f.
!$omp Parallel Default(PRIVATE) CopyIn(/TOCALC/,/OBSER/)
!$omp&Shared(PYJGX,WORKK,WORK)
!$omp&FirstPrivate(JSUB,SIGFAC,OFAC,NOBSER,NUMEQT,NDIM,MF)
!$omp&FirstPrivate(IterFirst,IterLast,NSTORE,NACTVE,MAXACT)
!$omp&FirstPrivate(NVAR,CORDEN,VALFIX,NRANFIX,RANFIXEST)
!$omp&FirstPrivate(RSCOPY,BSCOPY,INTLIST)
...
C   an explicite declaration of  REAL*8 W to SUBROUTINE NPAG. W should be
C   created w/random value, but is initialized prior to use.

C wmy2017Sep08
C   This Do is over less IG now
C	DO 800 IG=1,NACTVE
...
C   in MAKEVEC, but it can't hurt to init here.
        ...
        CALL MAKEVEC(NVAR,NOFIX,NRANFIX,IRAN,XXIG,VALFIX,RANFIXEST,
     1   PCOPY)

C wmy HERE ... There should be nothing wrong w/this call/code; Both
C  PX and W are privates here, and will have private copies under
C  IDPC. But in one iteration of the code, PX was "lost" w/values
C  being set to 0.0. PCOPY is an easier name to remember than
C  PX, i.e. PCOPY is a "Copy of P," meaning I can trace the behavior
C  of the variable through DO 800 easier than a variable named, PX.
...
C        write (*,*) "DO 800 CALL IDPC", NPX,NOBSER,NUMEQT,NDIM, MF

        CALL IDPC(JSUB,IG,NPX,PCOPY,W,NOBSER,NUMEQT,
...
C wmy2017Sep07
!$omp flush (WORK,WORKK,PYJGX,CORDEN)
!$omp critical
...
C     1  ,JSUB,IterFirst,IG,IterLast
        write (*,*) "DO 800 :: (JSUB,IG,W) ="
     1  ,JSUB,IG
     1  ,W,WORKK(IG),CORDEN(IG,NVAR+1),WORK(IG)
...
C wmy2017Sep08
      write (*,*) "Calling NOTINT immediately after DO 800"
...
cend optimization
      ...
      write (*,*) "Made it past optimization"
...
c  values.
        ...
        write (*,*) "Calculating expeted values for this cycle"
...
C  RANDOM VARIABLES WILL BE THE MEANS FROM THE JUST COMPLETED CYCLE.
     ...
     2  NDIM,MF,RTOL,ATOL,PCOPY,RSCOPY,BSCOPY,INTLIST)
...
C  PARAMETER (THOSE WHICH ARE UNKNOWN BUT THE SAME FOR ALL SUBJECTS).
      ...
      CALL MAKEVEC(NVAR,NOFIX,NRANFIX,IRAN,EXXX,VALFIX,RANFIXEST,PX)
         ...
         write (*,*) JSUB,"DO 6000 Calling IDCALCY", NVAR+NOFIX+NRANFIX
     1     ,NDIM
     2      ,PX(21),YPRED(1,2),NUMEQT,NOBSER,NDIM,MF,RTOL,ATOL,MF
...
C        CALL IDCALCY(NVAR+NOFIX+NRANFIX,NDIM,PX,YPRED,NUMEQT)
	CALL IDCALCY(JSUB,IG,NVAR+NOFIX+NRANFIX,NDIM,PX,YPRED,NUMEQT,
     1      NOBSER,MF,RTOL,ATOL,RSCOPY,BSCOPY,INTLIST)

         write (*,*) "#5454 Return from IDCALCY", JSUB
...
C  FROM NOW ON.
     ...
     1   NOMAXTIM(JSUB),TEND,TBEGG)
         ...
         write (*,*) "Return from CALCTPRED", JSUB
...
C wmy2017Sep20 -- call from the orignial npagranfix.f
C        CALL IDCALCYY(NVAR+NOFIX+NRANFIX,NDIM,PX,TPRED,NUMT(JSUB),
C     1  YYPRED,NUMEQT)

	CALL IDCALCYY(JSUB,IG,NVAR+NOFIX+NRANFIX,NDIM,PX,TPRED,
     1      NUMT(JSUB), YYPRED,NUMEQT,
     2      NOBSER,MF,RTOL,ATOL,RSCOPY,BSCOPY,INTLIST)

         write (*,*) "#5536 Return from IDCALCYY", JSUB
...
C wmy2017Sep12
     ...
     2  PYJGXX,NRANFIX,RANFIXEST,NOBSER,NUMEQT,
     3  NDIM,MF,RTOL,ATOL,RSCOPY,BSCOPY,INTLIST)
...
C  PARAMETER (THOSE WHICH ARE UNKNOWN BUT THE SAME FOR ALL SUBJECTS).
	...
	CALL IDCALCY(JSUB,IG,NVAR+NOFIX+NRANFIX,NDIM,PX,YPRED,NUMEQT,
     1      NOBSER,MF,RTOL,ATOL,RSCOPY,BSCOPY,INTLIST)
      ...
      write (*,*) "#5733 Returned from IDCALCY()"
...
C  PARAMETER (THOSE WHICH ARE UNKNOWN BUT THE SAME FOR ALL SUBJECTS).
      ...
      write (*,*) "DO 7000 CALL IDCALCYY", NVAR+NOFIX+NRANFIX,NDIM,
     1  PX(8),TPRED(5),NUMT(JSUB),NUMEQT,NOBSER,MF,RTOL,ATOL(1)

      CALL IDCALCYY(JSUB,IG,NVAR+NOFIX+NRANFIX,NDIM,PX,TPRED,
     1  NUMT(JSUB), YYPRED,NUMEQT,
     2  NOBSER,MF,RTOL,ATOL,RSCOPY,BSCOPY,INTLIST)

       write (*,*) "#5800 Returned from IDCALCYY"
...
C  THE ABOVE END DO IS FOR THE  DO IMAXTIM = 1,NOMAXTIM(JSUB)  LOOP.
 ...
 7000   CONTINUE
       ...
       write (*,*) "Passed 7000; writing DENSITY" 
...
C wmy2017Sep12
     ...
     2  CENTER,PYJGXX,NRANFIX,RANFIXEST,NOBSER,NUMEQT,
     3  NDIM,MF,RTOL,ATOL,RSCOPY,BSCOPY,INTLIST)
...
C wmy2017Sep19
        integer NDIM,MF
        REAL*8 atol(20), rtol
        REAL*8, dimension(20) :: RCOPY
        REAL*8, dimension(5000,34) :: RSCOPY
        REAL*8, dimension(20) :: BCOPY
        REAL*8, dimension(5000,7) :: BSCOPY
        integer, dimension(128) :: INTLIST
...
C        CALL IDPC(NPX,PX,W,NOBSER,NUMEQT)
        CALL IDPC(JSUB,IG,NPX,PX,W,NOBSER,NUMEQT,
     1    NDIM,MF,RTOL,ATOL,RSCOPY,BSCOPY,INTLIST)
...
c        CALL DVODE(DIFFEQ,NDIM,X,TIN,TOUT,ITOL,RTOL,ATOL,ITASK,ISTATE,
c     1            IOPT,RWORK,LRW,IWORK,LIW,JACOB,MF,RPAR,IPAR)

 
C *DECK DVODE_empty
*DECK DVODE_empty
      SUBROUTINE DVODE_E (F, NEQ, Y, T, TOUT, ITOL, RTOL,
     1    ATOL, ITASK, ISTATE, IOPT, RWORK, LRW, IWORK, LIW,
     2    JAC, MF, RPAR, IPAR)

      EXTERNAL F, JAC

c wmy2017Oct -- Updated input argument declarations
      DOUBLE PRECISION T, TOUT, RTOL
      INTEGER NEQ, ITOL, ITASK, LRW, LIW, MF, ISTATE, IOPT

      double precision, dimension(LRW) :: RWORK
      integer, dimension(LIW) :: IWORK

      double precision, dimension(20) :: Y, ATOL
      double precision, intent(IN), dimension(257) :: RPAR
      integer, dimension(257) :: IPAR

c wmy2017Oct -- New local variables
      integer JSUB, IG
...
c wmy2017Oct03 debug --serial works; parallel "illegal operation : 4"
c      write (*,*) "NEQ", NEQ,T,Y(1)
c potential fix will be to threadsafe Y -- in main this is X.
c alternatively, install threadsafe DVODE_F90_m
c NOTE that printing X() in SUBROUTINE USERANAL also caused an
c illegal operation: 4.  I fixed that one by changing the declaration
c of X()
...
C ,ipar(34),ipar(35)
         ...
         write (*,*) "In EMPTY DVODE"
         write (*,*) "RPAR", RPAR(1), RPAR(2)
         write (*,*) "MF,LIW,LRW,NEQ",MF,LIW,LRW,NEQ
         write (*,*) "ATOL",ATOL(1),ATOL(20)
         write (*,*) "ITASK,ISTATE,IOPT", ITASK,ISTATE,IOPT
         write (*,*) "RTOL,ITOL",RTOL,ITOL
         write (*,*) "JSUB, IG", JSUB, IG
         write (*,*) "TIN,TOUT",T,TOUT
         write (*,*) "Y",Y(1),Y(2),Y(3)
         write (*,*) "IWORK",IWORK(1)
c seem to not print:
         write (*,*) "RWORK",RWORK(1),RWORK(2)
...
*DECK DVODE_REAL
     ...
     1            ITASK, ISTATE, IOPT, RWORK, LRW, IWORK, LIW, JAC,
     2            MF, RPAR, IPAR)

c wmy2017Oct11 -- Original Header (mostly)
c      EXTERNAL F, JAC
c      DOUBLE PRECISION T, TOUT, RTOL, ATOL, RWORK 
c      INTEGER NEQ, ITOL, ITASK, ISTATE, IOPT, LRW, IWORK, LIW,
c      1        MF
c      DIMENSION ATOL(20), RWORK(LRW), IWORK(LIW)
c
c c wmy2017Oct
c      double precision, dimension(257) ::  RPAR
c      integer, dimension(257) ::  IPAR
c      double precision, dimension(20), intent(INOUT) :: Y

c wmy2017Oct11 -- New Header
      EXTERNAL F, JAC

      DOUBLE PRECISION, intent(INOUT) :: T
      DOUBLE PRECISION :: TOUT, RTOL
      INTEGER NEQ, ITOL, ITASK, ISTATE, IOPT, LRW, LIW, MF

      double precision, DIMENSION(LRW) :: RWORK
c      double precision, dimension(20) :: Y, ATOL
      double precision, dimension(20), intent(INOUT) :: Y
      double precision, dimension(20) :: ATOL
      integer, dimension(LIW) :: IWORK
...
c RPAR, ATOL, RTOL, IWORK, print out as 0.0
c JSUB, IG, ISTATE, ITASK, IOPT (IOPT=0), MF
c   LIW, LRW, NEQ  seem correct
c Seems to NOT print:
         write (*,*) "RWORK",RWORK(1),RWORK(2)
         write (*,*) "Y",Y(1),Y(2),Y(3)
         write (*,*) "TIN,TOUT",T,TOUT
...
c wmy2017Oct03 debug --serial works; parallel "illegal operation : 4"
c      write (*,*) "NEQ", NEQ,T,Y(1)
c DID NOT WORK :: potential fix will be to threadsafe Y -- in main this is X.
c alternatively, install threadsafe DVODE_F90_m

C
C trying to write out these variables seems to cause illegal instruction : 4
C T, TOUT, Y
C
C If these are not written out, program still gets illegal instruction error
C but later on ...
...
cdebug  write(6,*) i,psi(i,i),test,psi(i,i)/test
	write(6,*) i,psi(i,i),test,psi(i,i)/test
...
C
	SUBROUTINE CALCRF(NTOTPAR,VEC,FNTVAL,NUMEQT,YO,C0,C1,C2,C3,
     1    NDIM,MF,RTOL,ATOL,PCOPY,RSCOPY,BSCOPY,INTLIST)
	IMPLICIT REAL*8(A-H,O-Z)
...
C       CALL IDPC(NPX,PX,W,NOBSER,NUMEQT)
       ...
       write (*,*) "CALL IDPC",NPX,NOBSER,NUMEQT,NDIM,MF
