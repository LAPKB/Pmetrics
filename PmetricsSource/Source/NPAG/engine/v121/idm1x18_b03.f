...
!$omp Threadprivate(RSO,BSO,SIGO,Y,X)
...
C wmy2017Sep26
        if (RSCOPY(I,2*J) .ne. RS(I,2*J)) then
           write (*,*) "BSO init :: RS.ne.RSCOPY for", Jsub,IG
        endif
       END DO
      END DO
...
C wmy2017Sep26
        if (RSCOPY(KNS,I) .ne. RS(KNS,I)) then
           write (*,*) "R init :: RS.ne.RSCOPY for", Jsub,IG,KNS
        end if
      END DO
C wmy2017Sep29 --
...
C        IF(N .EQ. -1) CALL ANAL3(X,T,TOUT)
      ...
      write (*,*) JSUB,IG,"USER->FUNC ::",T,X(1),X(2),X(3)
...
c wmy2017Nov13 -- Call to DVSRCO
c        integer job ! is 1 or 2, so just pass this value in
        real*8, dimension(49),save :: RSAV
        integer, dimension(40), save :: ISAV
!$omp ThreadPrivate(RSAV,ISAV)
...
c       write (*,*) "In USERANAL"
      ...
      write (*,*) JSUB,IG, "FUNC->DVODE",
     1  TIN,X(1),X(2),X(3),TOUT,RTOL,ATOL(1)

c wmy2017Nov11 -- Call DVSRCO; note first call is going to be w/random values.
        CALL DVSRCO(RSAV,ISAV,2)
        CALL DVODE(DIFFEQ,NDIM,X,TIN,TOUT,ITOL,RTOL,ATOL,ITASK,ISTATE,
     1            IOPT,RWORK,LRW,IWORK,LIW,JACOB,MF,RPAR,IPAR)
        CALL DVSRCO(RSAV,ISAV,1)
