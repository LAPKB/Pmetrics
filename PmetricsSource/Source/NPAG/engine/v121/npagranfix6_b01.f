...
!$omp&Shared(MAXTHREAD,NOFIX,RANFIXEST,NRANFIX,IMUSTEXIT)
...
C wmy2017Seop08 This replaces the GO TO 800 just prior to the
C   original label 700
C !$omp critical
C           IMUSTEXIT = IMUSTEXIT + 1
C !$omp end critical
...
C
C wmy2017Sep08 no need for this anymore 
...
C wmy2017Sep07 -- I tried this logic, but it didn't work so I
C   reversed the changes.  I then verified I put code exactly
C   back to original by runnign in serial and comparing the
C   DEN0001 and OUTT0001 to the control files.  Exactly the same!
C
C Following ENDIF and IF seem unnecessary. 
C
C rational: Logic is supposed to execute from 700 to end of DO 800
C for all IG, but is supposed to execute from here up to 700 for
C only IG > NSTORE. So we can move the ENDIF from here to just prior
C to 700. We can also ignore any decisions at 700 b/c all IG should
C execute past line 700 to the end of DO 800.
C
C Unfortunately, above rational is wrong -- even in single thread
C the progam crashes w/Pr(1|G) = 0 on first cycle, i.e. immediately
C on leaving the DO 800 
C
	ENDIF 

C
C wmy 2017Sep08 Move this into a separate DO loop preceding DO 800
C   This move is because these lines can not be parallelized -- 
C   WORK and WORKK are shared variables -- overhead of parallelizing
C   access is slower than just serially processing
C	IF(IG .LE. NSTORE) GO TO 700
C wmy 2017Sep08 Second	try; move the 700 stuff to this point
C   and continue the loop afterward.
C !$omp critical
C !$omp flush (WORK,WORKK,PYJGX,CORDEN)
C	IF(IG .LE. NSTORE) then
C
C Following line is label 700, copied here, instead
C          WORK(IG)=PYJGX(JSUB,IG)*CORDEN(IG,NVAR+1)
C
C	  WORKK(IG) = PYJGX(JSUB,IG)
C          cycle
C        endif
C !$omp end critical
...
C wmy 2017Sep08
C If new logic is correct, then there is no need for this GO TO 800
C but move it to the end of the DO 800 just b/c MvG must have at some
C point had a run that set NACTVE > MAXACT
C
C wmy 2017Sep08 Need to gather threads before exiting
C	 GO TO 800
C Since logic is incorrect, leave it in place; but can not jump in
C or out of a omp critical section, so we have to also comment out
C the surrounding omp ciritical directives (or replace the GO TO 800
C logic; which we do by gathering threads and checking the IMUSTEXIT
C counter).
	ENDIF
...
C wmy 2017Sep07
C   The following ELSE closes the initial IF (IG > NSTORE)
C See discussion up there. For some reason we need the GO TO 700
C	ELSE 
C
...
C        ENDIF
C wmy 2017Sep07
C   The above ENDIF closes IF (IG > NSTORE) {calculate new PYJGX}
C   ELSE {Read the PYJGX from the last loop} ENDIF

C wmy2017Sep08 ... Yes, this seems silly and impossible; but MvG
C   must have put this decision (above) for a reason; so I moved 
C   it here.
C
C wmy2017Sep08 Decision is replaced by IMUSTEXIT
C	IF(IG .GT. MAXACT) THEN
C          GO TO 800
C !$omp barrier            
C        IF (IMUSTEXIT .GT. MAXACT) then
C           exit
C        ENDIF
C The intent of above is to gather all threads, if the total 
C  threadcount through the loop has hit MAXACT, then we exit
C  the loop.  SHould be replaced with a DO WHILE
