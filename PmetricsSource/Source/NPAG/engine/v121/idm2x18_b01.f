...
C  AT ESTML(I) AS DEFINED ABOVE.
	...
	CALL EVAL2(JSUB,IG,NPP,ESTML,YPRED,NUMEQT,
     1      NOBSER, NDIM,MF,RTOL,ATOL,INTLIST)
...
C	SUBROUTINE EVAL2(YPRED,NUMEQT)
	SUBROUTINE EVAL2(JSUB,IG,NPP,ESTML,YPRED,NUMEQT,
     1      NOBSER,NDIM,MF,RTOL,ATOL,INTLIST)
...
C  DIRECTLY TO SUBROUTINE FUNC2 VIA COMMON/PARAMD.
	...
	IMPLICIT REAL*8(A-H,O-Z)
	COMMON/SUM2/ M,NPNL
C wmy2017Sep13 Not sure why /CNST2/ is declared here as it
C   is not used here.
C        COMMON/CNST2/ NPL,NOS,NDRUG,NADD
	DIMENSION YPRED(594,NUMEQT)
        ...
        real*8, dimension(32) :: ESTML
         ...
         write (*,*) "In EVAL2 :: Calling FUNC2(M,NOBSER,NUMEQT,NPP)",
     1    M, NOBSER,NUMEQT,NPP
