...
c wmy2017Oct25 -- IRAN is in COMMON/TOCALC/ and has "random" or at
c least unexpected behavior inside the parallel region. So, make a
c FirstPrivate variable, IRANCOPY
        do iparam=1,32
           IRANCOPY(iparam) = IRAN(iparam)
        end do

c wmy2017Oct20 -- removed the CopyIn clause from
c !$omp Parallel Default(PRIVATE) CopyIn(/TOCALC/,/OBSER/)
c CopyIn initializes Threadprivate variables w/master
c thread values; this is not necessary for /TOCALC/ and
c /OBSER/ as the variables in them have been assigned to
c local variables and passed to downstream subroutines.

C  !$omp&Shared(MAXTHREAD)
c
! NEW PARALLEL CODE BELOW AS OF npageng28.f.
!$omp Parallel Default(PRIVATE) Shared(PYJGX,WORKK,WORK)
!$omp&FirstPrivate(CORDEN,JSUB,SIGFAC,OFAC,NOBSER,NUMEQT)
!$omp&FirstPrivate(NDIM,MF,IterFirst,IterLast,NSTORE,NACTVE,MAXACT)
!$omp&FirstPrivate(RTOL,ATOL,RSCOPY,BSCOPY,INTLIST,VALFIX,RANFIXEST)
!$omp&FirstPrivate(NVAR,NRANFIX,IRANCOPY)
C   ABOVE LINE EDITED AS OF npagranfix5.f.
c
c  These are still defined AND used by the analytic routines. So
c  IDPC must receive copies of them, not the real thing!
c     COMMON/PARAMD/P
c     COMMON/INPUT/ R,B
c     COMMON/TOCALC/IRAN,PX,NOFIX,NSUB,gamma,flat,AB
...
C   in MAKEVEC, but it can't hurt to init here.
        ...
        CALL MAKEVEC(NVAR,NOFIX,NRANFIX,IRANCOPY,XXIG,VALFIX,RANFIXEST,
     1   PCOPY)
...
C wmy2017Sep12 Added /TOUSER/ varbs to CALL IDPC()
      ...
      write (*,*) JSUB, IG, "DO 800 CALL IDPC", PCOPY(1), PCOPY(2),
     &  PCOPY(3), PCOPY(4), PCOPY(5)
