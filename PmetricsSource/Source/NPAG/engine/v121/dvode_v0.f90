...
! wmy2017Oct13 --New local variables
      ...
      real(dp), dimension(1):: RTOL  ! int in main must be RTOL(:) here
...
!  at first reference:
         ...
         write (*,*) JSUB, IG, "RTOL(1),ITOL",RTOL(1),ITOL
...
! the code returns as expected.
      IF (TOUT.EQ.T) then
        write (*,*) JSUB,IG,"returning from DVODE_v01.f90 w/T=TOUT=",T
        RETURN
      end if
!-----------------------------------------------------------------------
! Block B.
