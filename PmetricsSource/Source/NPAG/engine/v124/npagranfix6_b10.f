...
C wmy2017Sep08 ... to get thead num in DO 800
        USE OMP_LIB
        USE npag_utils, only: makevec,check_input_array_size
     ...
     5   ,i_errmod,i_is_poisson_obs,i_is_log10
...
C  REMAINS THE MAX. NO. OF OBSERVATION TIMES (AS OF NPAG113.FOR).
     ...
     1 TPRED(71281),YYPRED(71281,NUMEQT),
     2 C0P(NUMEQT),C1P(NUMEQT),C2P(NUMEQT),C3P(NUMEQT),
     3 C0(NUMEQT),C1(NUMEQT),C2(NUMEQT),C3(NUMEQT),
...
C !$omp ThreadPrivate(/OBSER/,/TOCALC/,/INPUT/)
...
C        write (*,*) "Ret. from FILRED() nr. #3660"

        call cp_lrcs_to_rpar(gamma,flat,numeqt,c0,c1,c2,c3,rpar)
!         RPAR(k_gamma) = gamma
!         do III=1,numeq
...
C--------------------------------- Start Poisson
             ...
             if (C1(J).eq.-10) IPAR(i_is_log_10+J) = -10
C--------------------------------- End Poisson
          else
C--------------------------------- Start NORMAL
...
C New Code, Assumes Mix of ~ Poisson and ~ Normal
          ...
          IF(RPAR(k_sum_z_sq) .LE. 22708.D0) THEN
           IF(IG .LE. MAXACT) THEN
            PYJGX(JSUB,IG) = RPAR(k_prod_pr)
     1        * DEXP(-.5D0*RPAR(k_sum_z_sq))/RPAR(k_sfac)/RPAR(k_ofac)
           ENDIF
           WORKK(IG) = RPAR(k_prod_pr)
     1        * DEXP(-.5D0*RPAR(k_sum_z_sq))/RPAR(k_sfac)/RPAR(k_ofac)
          ENDIF
...
C  RANDOM VARIABLES WILL BE THE MEANS FROM THE JUST COMPLETED CYCLE.
     ...
     1  CALCRF,0,ICONV,NITER,ICNT,NUMEQT,YO,C0,C1,C2,C3,NBCOMP,
...
C             NPOISSONOBS=NPOISSONOBS+1
             ...
             SIG(I,J)=1.D0
             IPAR(i_is_poisson_obs+J)=229
             if (C1(J).eq.-10) IPAR(i_is_log10+J) = -10 
...
C      PARAMETER(MAXNUMEQ=7)
     ...
     2 C0(NUMEQT),C1(NUMEQT),C2(NUMEQT),C3(NUMEQT),
