c wmy2017Jul26
c
c Separated npagranfix6.f into the files:
c   npagranfix6.f
c   emint.f
c   vodetot.f
c   lapack.f
...
C wmy2017Jul31 -- These carry the DVODE params and other vals, too
C   The first 22 words are reserved for dvode_f90_m
C        double precision, dimension(128) :: RPAR
C        integer, dimension(128) :: IPAR
        integer LengthOfPX

C wmy2017Jul18 -- Logical :: if (IgIsGoodPoint.ne.1) Set support point prob to 0
...
!$omp&Shared(MAXTHREAD,NOFIX,RANFIXEST,NRANFIX,ICYCLE)
...
c wmy2017Aug02 Assume the support is feasible, if it should be
C   initialized, other than reading in P, then do so.
         IgIsGoodPoint = 1
...
C wmy2017Jul31 PX returned as list of all params in the order 
C   declared in pmetrics::input::model.txt
C note: I moved rpar declaration to idm modules.
C        do iii=1:(NVAR+NOFIX+NRANFIX)
C          rpar(iii+22) = PX(iii)
C        end do
C        ipar(23) = JSUB
C        ipar(24) = IG
C        ipar(27) = NVAR
...
C        ipar(30) = NACTVE
        ...
        LengthOfPX = NVAR+NOFIX+NRANFIX
	CALL IDPC(PX,W,IgIsGoodPoint,ICYCLE,JSUB,IG,LengthOfPX)
C     1   rpar, ipar) 

C        SUBROUTINE IDPC(X,SUMSQJ,IgIsGoodPoint,ICYCLE,JSUB,IG,
C     1   LengthOfPX)
...
C Print progress out here wmy
        write (*,*) "JSUB=",JSUB,"IG=",IG,
     1    "Of ",NACTVE,"IS",NACTVE,"w/SSQ=",W,
     2    "P(YJ|X)=",PYJGX(JSUB,IG),"P(YJ,X)=",
     3    WORKK(IG)*CORDEN(IG,NVAR+1)
...
c wmy HERE HERE; debugging, since prior w/12 pts is entered, this
C  print should just verify what we can almost figure out by
C inspection
      if (ICYCLE.eq.1851) then
        write (*,*) "1: emint returns X ::"
	DO 7317 IG=1,NACTVE
          write (*,*) corden(IG,nvar+1)
 7317   enddo
      endif
...
C HERE HERE
      if (ICYCLE.eq.1851) then
        write (*,*) "0 : keep=",keep,"emint returns X ::"
	DO 7319 IG=1,NACTVE
          write (*,*) corden(IG,nvar+1)
 7319   enddo
      endif
...
C        ipar(29) = NRANFIX
C        ipar(30) = NACTVE
C wmy2017Jul31 -- Above rpar and ipar inits do not need to occur here.
C   At this point we only care that X gets ORDERRED correctly -- no
C   calculations on PX will be done. SR ELDERY() calls SR IDPC(), and
C   from inside ELDERY the NRANFIX variables will be changing value.
...
C        ipar(29) = NRANFIX
C        ipar(30) = -99
Cwmy2017Jul31 -- These are posterior predictions, there is no specific point!
C  therefore, set ipar(24) = ICENTER, instead of IG, and set ipar(30) = -99,
C  instead of nactve.
...
C        ipar(30) = NACTVE
        ...
        LengthOfPX = NOFIX+NRANFIX+NACTVE
	CALL IDPC(PX,W,IgIsGoodPoint,-17,JSUB,IG,LengthOfPX)
C     1    ,rpar, ipar) 

C wmy in above call to idpc() ICYCLE is set to -17
C        SUBROUTINE IDPC(X,SUMSQJ,IgIsGoodPoint,ICYCLE,JSUB,IG,
C     1   LengthOfPX)
...
C wmy2017Jul31
       integer IgIsGoodPoint,LengthOfPX,IsNMpoint
C       double precision, dimension(128) :: rpar
C       integer, dimension(128) :: ipar
...
C wmy2017Jul31 -- makevec() is not called b/c the support point is
C   manufactured in subroutine ELDERY(). Because there is only one
C   point, there is no need to sequentially process a vector of
C   active points.  In case you need to tell ODE this, pass it the
C   variable IsNMPoint; if (IG == IsNMPoint) then ODE call originated
C   from ELDERY()
C
        LengthOfPX = NOFIX+NRANFIX+NACTVE
        IsNMPoint = -8
	CALL IDPC(PX,W,IgIsGoodPoint,IsNMPoint,JSUB,-19,LengthOfPX)
C , 1    rpar, ipar) 
C wmy in above call to IDPC test of point is replaced by flag
C  indicating this is a NelderMead point, and IG is replaced by
C  anotehr flag, indicating ... not sure what!
C        SUBROUTINE IDPC(X,SUMSQJ,IgIsGoodPoint,ICYCLE,JSUB,IG,
C     1   LengthOfPX)
...
C       CALL ELDERY(NRANFIX+NVAR,START,OPTVAR,VALMIN,1.D-10,STEP,1000,
C     1  CALCRF,0,ICONV,NITER,ICNT,NUMEQT,YO,C0,C1,C2,C3)
