C wmy2017Jul26 -- Need to separate out the various packages for
C  upgrade to f90+ This file contains the DVODE package, and some
C  supporting functions.
C
C Note: gcc supports f95+
C
C wmy2017Jul26 -- Declare RPAR and IPAR as unknown sized arrays
C  Note: The Intel compiler used to complain about this, if these
C  arrays were not declared in main -- however, here forward, we
C  will be getting rid of COMMON blocks in main, and RPAR will
C  be used, instead.  Also, DVODE_f90_m.f90 (the threadsafe version
C  of DVODE is vetted on the intel compiler.) ...
C wmy2017Aug01 -- Declare RPAR of fixed dimension 128 (for debugging)
C  but it didn't seem to matter.
...
*DECK DVODE
     ...
     1        MF, IPAR
      DIMENSION Y(*), ATOL(*), RWORK(LRW), IWORK(LIW), RPAR(128),
     1        IPAR(128)
...
*DECK DVSTEP
     ...
     1   ACOR(*), WM(*), IWM(*), RPAR(128), IPAR(128)
C-----------------------------------------------------------------------
