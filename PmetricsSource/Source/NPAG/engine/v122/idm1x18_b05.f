...
C wmy 2017Sep12
	CALL SUMSQ(JSUB,IG,NPX,X,SUMSQJ,NOBSER,NUMEQT,
     1   NDIM,MF,RTOL,ATOL,P,RSCOPY,BSCOPY,INTLIST,IPAR,ObsError,RPAR)
...
C wmy2017Sep12 Receiving /TOUSER/ varbs
     ...
     1   NDIM,MF,RTOL,ATOL,P,RSCOPY,BSCOPY,
...
!$omp Threadprivate(/BOLUSCOMP/,/OBSER/)
     ...
     1 YO(594,MAXNUMEQ), BS(5000,7)
C      , STDEV(594,MAXNUMEQ)
...
C      Save RSO,BSO,SIGO,Y,X
       ...
       integer, save :: IKNS,KNS,KNT,NDO,KNSOLD
!$omp Threadprivate(IKNS,KNS,KNT,NDO,KNSOLD)
...
C      integer NNORMALOBS, NPOISSONOBS, MISVAL
C      real*8 SIGFAC,OFAC
...
C-----------------------------------------------------
       ...
       write (*,*) "*** wmy2017Dec05 MvG code validation required."
...
C        DO J=1,NOS
C          DO I=1,M
C            IF(YO(I,J) .EQ. -99) F((J-1)*M+I) = 0.D0
C            IF(YO(I,J) .NE. -99) F((J-1)*M+I) =(Y(I,J)-YO(I,J))
C     1                                         /ObsError(I,J)
        DO J=1,INTLIST(9)
         DO I=1,INTLIST(10)
	  IF(YO(I,J) .EQ. -99) F((J-1)*INTLIST(10)+I) = 0.D0
          IF(YO(I,J) .NE. -99) F((J-1)*INTLIST(10)+I) =(Y(I,J)-YO(I,J))
     1                                         /ObsError(I,J)

C wmy DEBUG
C          write (*,*) "idm01",I,J,ZSCORE,RPAR(129)
C     1       , Y(I,J), YO(I,J), ObsError(I,J), YO(I,J), Y(I,J)
...
C          if(YO(I,J) .EQ. -99) then
C           F((J-1)*INTLIST(10)+I) = 0.D0
C           MISVAL = MISVAL+1
...
C           IF (IPAR(120 + J) == 10) THEN  ! Y is log10(X) and YO(I,J) are log10(obs)
C            YOBSERVED = 10^(YO(I,J))     ! log10(obs) enterred in data.csv
C            YMEAN = 10^Y(I,J)            ! log10(X) happenned in call to OUTPUT
C           ELSE
C            YOBSERVED = YO(I,J)
C            YMEAN = Y(I,J)
C           ENDIF
...
C YOBSERVED ~ POISSON
C           IF (IPAR(110 + J) == 229) THEN ! 229 == extended ASCII code for lower case lambda; USE Poisson
C
C C            Poisson: p( YO(i,J) | Y(I,J) ) =  exp( -1 * Y(I,J) ) * ( Y(I,J)^YO(I,J) / fact(YO(I,J)) ))
C
C              P_thresh = 30.0
C
C              YOBSERVED = YOBSERVED    ! fact reqs nint(); but fact(N) = Gamma(N + 1)
C              ObsError(I,J) = sqrt( YMEAN )  ! Poisson w/mean Y; OK to be a REAL value
C
C              IF (YMEAN > P_thresh) THEN        !  Normal approx. is used for P_thresh = 60
C                 F((J-1)*INTLIST(10)+I) = (YOBSERVED - YMEAN) / ObsError(I,J)
C                 SIGFAC = SIGFAC * ObsError(I,J)
C                 NNORMALOBS = NNORMALOBS+1
C              ELSE ! F equals the Poisson probability, not (y - yo)/sigma
C                 F((J-1)*INTLIST(10)+I) = exp( -1 * YMEAN ) * ( YMEAN^YOBSERVED ) / gamma( YOBSERVED + 1.0D )
...
C              ObsError(I,J) = RPAR(J+97) + RPAR(J+104)*YMEAN
C     1            + RPAR(J+111)*YMEAN*YMEAN + RPAR(J+118)*YMEAN**3
...
C
C              if(IPAR(38).eq.2) ObsError(i,j) = ObsError(i,j)*RPAR(96)
C              if(IPAR(38).eq.4) ObsError(i,j) = RPAR(96)*RPAR(97)
C              if(IPAR(38).eq.3) ObsError(i,j) = dsqrt(ObsError(i,j)**2
C     1               + RPAR(96)**2)

C-------------- 
C M            IF(SIG(I,J) .EQ. 0) THEN  {} ! replaced with:
C              IF(ObsError(I,J) .EQ. 0) THEN
C                WRITE(*,2345) JSUB
C                WRITE(25,2345) JSUB
C2345            FORMAT(//' A S.D. IS 0 FOR JSUB = ',I5,'. RERUN THE '/
C     1' PROGRAM WITH C0 NOT = 0  FOR THIS SUBJECT, OR WITH THIS'/
C     2' SUBJECT ELIMINATED.')
C                CLOSE(27)
C                CLOSE(25)
...
C              IF(ObsError(I,J) .LT. 0) THEN
C                WRITE(*,2346) JSUB
C                WRITE(25,2346) JSUB
C2346            FORMAT(//' A S.D. < 0 FOR JSUB = ',I5,'. RERUN THE '/
C     1' PROGRAM WITH A BETTER CHOICE FOR THE ASSAY ERROR POLYNOMIAL'/
C     2' COEFFICIENTS.')
C                CLOSE(27)
C                CLOSE(25)
...
C--------------  Update F(), SIGFAC, and NNORMALOBS

C              F((J-1)*INTLIST(10)+I) =(YOBSERVED(I,J)-YMEAN(I,J))
C     1                                         /ObsError(I,J)
...
C           endif ! if(Poisson){...} else {Normal}

C          ENDIF ! if(YO(I,J) .EQ. -99){...} else {YO ~ {Normal,Poisson}}

C         END DO ! number of measurements
C        END DO ! number of output equations
...
C        OFAC=2.506628274631**NNORMALOBS
...
C        RPAR(127) = OFAC
C        IPAR(39) = MISVAL
C        IPAR(40) = NNORMALOBS
C        IPAR(41) = NPOISSONOBS
...
C wmy2017Sep12 Passing in /TOUSER/ variables
     ...
     1   NDIM,MF,RTOL,ATOL,P,RSCOPY,BSCOPY,INTLIST,IPAR,ObsError,
     2   RPAR) 
...
C--   If IPAR(100).eq.0 then the point (JSUB,IG) is "bad" according to a
...
c   will automatically IPAR(100)<-0 if DVODE returns an error.
c
        if (IPAR(100).eq.0) then
          write (*,*) "...(!)... at",JSUB,IG,"IPAR(100)=",IPAR(100)
          SSQ = 10.73d99
          return
        endif
C--
...
C-- New code to support obs ~ Poisson
...
C          IF (IPAR(110 + J) == 229) THEN
C           if RPAR(129) > 0.D0)
C            RPAR(129) = RPAR(129) * F(IinF)
C           else
...
C           RPAR(128) = RPAR(128) + F(IinF) * F(IinF)
C          ENDIF
C          IinF = IinF + 1
