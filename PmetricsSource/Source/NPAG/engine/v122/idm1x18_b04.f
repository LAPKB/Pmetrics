...
32      IF(NDIM .NE. -1) then
     ...
     1      NDIM,MF,RTOL,ATOL,P,R,INTLIST,IPAR)
...
C Poisson 
C
C Strategy: In model.txt, for each of the 7 output equation set IPAR to flag:
C    Poisson
C    N trials averaged per reporated observation
C    Log10 or absolute measurement
C
C Equations:
C
...
C
C	 if(YO(I,J) .EQ. -99) then    ! This "obs" doesn't count toward fobj!
C           F((J-1)*INTLIST(10)+I) = 0.D0
C        else
C           IF (IPAR(120 + J) == 10) THEN    ! Y and YO(I,J) are log10(obs); log10(Y) happenned in #OUTPUT
C              YOBSERVED = 10^(YO(I,J))
C              YMEAN = 10^Y(I,J)
C           ELSE
C              YOBSERVED = YO(I,J)
C              YMEAN = Y(I,J)
C           ENDIF
C
C           IF (IPAR(100 + J) == 229) THEN ! 229 == extended ASCII code for lower case lambda
C
C           YOBSERVED = nint(YOBSERVED)    ! must be int for fact
C
C              N = IPAR(110 + J)
C              YMEAN = N * YMEAN
C              YSIGMA = sqrt( YMEAN )    ! Poisson w/mean N*observed
C
C              IF (YMEAN > P_thresh) THEN        !  Normal approx. is used for P_thresh = 60
C                 F((J-1)*INTLIST(10)+I) = (YOBSERVED - YMEAN) / YSIGMA
C              ELSE ! Poisson is used p( YO(i,J) | Y(I,J) ) =  exp( -1 * Y(I,J) ) * ( Y(I,J)^YO(I,J) / fact(YO(I,J)) ))
C                 YOBSER = ceiling(YOBSER)
C                 F((J-1)*INTLIST(10)+I) = exp( -1 * YMEAN ) * ( YMEAN^YOBSER) / gamma(YOBSER - 1.0D)
C              ENDIF
C
C           ELSE ! Normal will be used
...
C STDEV - Above will need to be replaced with a function or subroutine call.
C
...
c 
!$omp ThreadPrivate (RWORK, IWORK, RPAR)
        save RWORK, IWORK, RPAR
