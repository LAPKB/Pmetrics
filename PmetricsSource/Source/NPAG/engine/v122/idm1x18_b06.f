...
!  Poisson probs is returned in RPAR(129)
...
C wmy2017Sep26
        if (RSCOPY(I,2*J) .ne. RS(I,2*J)) then
           write (*,*) "BSO init :: ERROR: RS.ne.RSCOPY for",
...
C wmy2017.07.11 debug
         write (*,*) "Enterred FUNC: RSO and SIGO init", JSUB, IG,
     1    NDO, NI, INTLIST(8),intlist(7)
C
...
C         CALL SHIFT(TLAG,NDO,SIGO,NDRUG,NADD,RSO)
          CALL SHIFT(TLAG,NDO,SIGO,INTLIST(5),INTLIST(6),RSO)
...
C
C debug
          write (*,*) "DEBUG 2018.06.15 Called SHIFT(): TLAG,NDO,NDRUG,N
     1ADD,NI,ND,NOS,M",TLAG(1),NDO,INTLIST(5),INTLIST(6),INTLIST(7)
     2,INTLIST(8),INTLIST(9),INTLIST(10)
...
C     1            + RPAR(J+111)*YMEAN*YMEAN + RPAR(J+118)*YMEAN**3
              ObsError(I,J) = RPAR(J+97) + RPAR(J+104)*YOBSERVED
     1            + RPAR(J+111)*YOBSERVED*YOBSERVED
     2            + RPAR(J+118)*YOBSERVED**3
...
C M       if(ierrmod.eq.4) sig(i,j) = gamma*flat
              ...
              if(IPAR(38).eq.2) ObsError(i,j) = ObsError(i,j)*RPAR(96)
              if(IPAR(38).eq.4) ObsError(i,j) = RPAR(96)*RPAR(97)
              if(IPAR(38).eq.3) ObsError(i,j) = dsqrt(ObsError(i,j)**2
     1               + RPAR(96)**2)
...
C back up to main.
C
C RPAR(129) = \prod Poisson Probs
C RPAR(128) = summsq Normalized deviations for Y_obs ~ Normal
        RPAR(126) = SIGFAC
        RPAR(127) = OFAC
        IPAR(39) = MISVAL
        IPAR(40) = NNORMALOBS
...
c          write (*,*) "...(!)... at",JSUB,IG,"IPAR(100)=",IPAR(100)
          SSQ = 10.73d99
          return
        endif
...
C           RPAR(129) = RPAR(129) + F(IinF) * F(IinF)
C          ENDIF
C          IinF = IinF + 1
