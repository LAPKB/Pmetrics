...
c wmy2017Sep08 DO 800 variables
       integer IterFirst, IterLast, NInDO800
...
!$omp ThreadPrivate(ObsError,IPAR)
...
c
! NEW PARALLEL CODE BELOW AS OF npageng28.f.
!$omp Parallel Default(PRIVATE) Shared(PYJGX,WORKK,WORK,NInDO800,CORDEN)
!$omp&CopyIn(IPAR,ObsError)
...
c  immediately call FUNC2() or FUNC3(). Here, teh ANAL3 routine still
c  requires that /PARAMD/P be set
...
c     &  PCOPY(3), PCOPY(4), PCOPY(5)
     ...
     1    NDIM,MF,RTOL,ATOL,RSCOPY,BSCOPY,INTLIST,IPAR,ObsError)
C
...
C     1  YYPRED,NUMEQT)
	...
	CALL IDCALCYY(JSUB,IG,NVAR+NOFIX+NRANFIX,NDIM,PX,TPRED,
     ...
     2      NOBSER,MF,RTOL,ATOL,RSCOPY,BSCOPY,INTLIST,IPAR,ObsError)
...
C wmy2017Sep12
     ...
     3  NDIM,MF,RTOL,ATOL,RSCOPY,BSCOPY,INTLIST,IPAR,ObsError)
...
c  immediately call FUNC2() or FUNC3(). 
	...
	CALL IDCALCY(JSUB,IG,NVAR+NOFIX+NRANFIX,NDIM,PX,YPRED,NUMEQT,
     1      NOBSER,MF,RTOL,ATOL,RSCOPY,BSCOPY,INTLIST,IPAR)
