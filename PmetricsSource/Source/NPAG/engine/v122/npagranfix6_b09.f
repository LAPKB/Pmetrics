...
C  REMAINS THE MAX. NO. OF OBSERVATION TIMES (AS OF NPAG113.FOR).
     ...
     3 TPRED(71281),YYPRED(71281,NUMEQT),C0P(NUMEQT),
...
c wmy2017Dec01 DO 1000 variables
       integer NInDO1000
       real PInDO1000
       double precision, dimension(594,NUMEQT) :: YPRED
...
c wmy2017Sep08 DO 800 variables
       integer IterFirst, IterLast, NInDO800, NNInDO800
       integer NPX, NOFIXCOPY
       integer, dimension(32) :: IRANCOPY
       real*8 W, XXIG(30)
       real    PInDO800
       double precision, save, dimension(594,MAXNUMEQ) :: ObsError
...
!$omp ThreadPrivate(ObsError,IPAR,RPAR)
...
C        write (*,*) "Ret. from FILRED() nr. #3600"
...
C  TIME I. IN THIS CASE, IGNORE THIS Y AND INCREASE MISVAL BY 1.
	  ...
	  ENDIF
          ...
          if (C0(J).eq.-299.and.C2(J).eq.-299
     1   .and.C3(J).eq.-299) then 
C--------------------------------- Start Poisson
...
!$omp&Shared(PYJGX,WORKK,WORK,NInDO800,NNInDO800,CORDEN)
!$omp&CopyIn(ObsError,IPAR,RPAR)
!$omp&CopyIn(/OBSER/,/TOCALC/,/PARAMD/,/INPUT/)
!$omp&FirstPrivate(JSUB,SIGFAC,OFAC,NOBSER,NUMEQT,PInDO1000)
!$omp&FirstPrivate(NDIM,MF,IterFirst,IterLast,NSTORE,NACTVE,MAXACT)
!$omp&FirstPrivate(RTOL,ATOL,INTLIST,VALFIX,RANFIXEST,RSCOPY,BSCOPY)
!$omp&FirstPrivate(NVAR,NRANFIX,IRANCOPY,NOFIXCOPY)
C  !$omp&num_threads(2)
...
C     &  PCOPY(3), PCOPY(4), PCOPY(5)
     ...
     1    NDIM,MF,RTOL,ATOL,RSCOPY,BSCOPY,INTLIST,IPAR,ObsError,RPAR)
C
...
C
C          IF(RPAR(128) .LE. 22708.D0) THEN
C           IF(IG .LE. MAXACT) THEN
C            PYJGX(JSUB,IG) = RPAR(129)
C     1        * DEXP(-.5D0*RPAR(128))/RPAR(126)/RPAR(127)
C           ENDIF
C           WORKK(IG) = RPAR(129)
C     1        * DEXP(-.5D0*RPAR(128))/RPAR(126)/RPAR(127)
C          ENDIF
...
C          write (*,*) "TID", TID, "returns", work(IG), WORKK(IG) 
          ...
          write (*,*) "      ---  END DO 800 :: (JSUB,IG,w,PYJGX,px) ="
     1    ,JSUB,IG
     1    ,W,WORKK(IG),CORDEN(IG,NVAR+1)

c wmy201722Dec -- Need to restore option to skip bad points
c wmy2017Jul26 -- I think go to is safe here ...
C if (IgIsGoodPoint.eq.0) then ; IgIsGoodPoint replaced by IPAR(100)
C
        else
...
c  values.
        ...
        write (*,*) "DO 1100 :: Calculate expected values"
...
C      WRITE(25,6093) SDEST1,SDEST2,SDEST3,SCALINFO
 ...
 6090  CONTINUE
        ...
        write (*,*) "6090 Calculated Means, Medians, and Modes"
...
C        CALL IDCALCY(NVAR+NOFIX+NRANFIX,NDIM,PX,YPRED,NUMEQT)
	CALL IDCALCY(JSUB,IG,NPX,NDIM,PCOPY,YPRED,NUMEQT,
     1      NOBSER,MF,RTOL,ATOL,RSCOPY,BSCOPY,INTLIST,IPAR,ObsError,
     2      RPAR)
...
c     1    ,NOBSER,MF,RTOL,ATOL,RSCOPY,BSCOPY,INTLIST,IPAR,ObsError)
         ...
         write (*,*) "DO 6000 Return from IDCALCY", JSUB, ICENTER
...
C  FROM NOW ON.
     ...
     1   NOMAXTIM(JSUB),TEND,TBEGG)
         ...
         write (*,*) "Return from CALCTPRED", JSUB, ICENTER
...
C	CALL IDCALCYY(JSUB,IG,NVAR+NOFIX+NRANFIX,NDIM,PX,TPRED,
	CALL IDCALCYY(JSUB,IG,NPX,NDIM,PCOPY,TPRED,
     1      NUMT(JSUB), YYPRED,NUMEQT,NOBSER,
     2      MF,RTOL,ATOL,RSCOPY,BSCOPY,INTLIST,IPAR,ObsError,RPAR)

        write (*,*) "#5789 Return from IDCALCYY: JSUB, ICENTER",
     1      JSUB, ICENTER
...
C wmy2017Sep12
     ...
     3  NDIM,MF,RTOL,ATOL,RSCOPY,BSCOPY,INTLIST,IPAR,ObsError,RPAR)
...
C  FOR THE MEANS, MEDIANS, AND MODES.
        ...
        DO ICENTER = 1,3
          ...
          write (*,*) "EXX filled for ICENTER", ICENTER
...
C        DO I=1,NVAR+NOFIX+NRANFIX
      ...
      write (*,*) "Param VECs filled; CALLing IDCALCY"

	CALL IDCALCY(JSUB,IG,NPX,NDIM,PCOPY,YPRED,NUMEQT,
     1      NOBSER,MF,RTOL,ATOL,RSCOPY,BSCOPY,INTLIST
     2      ,IPAR,ObsError,RPAR)
...
C  STORE YPRED INTO YPREDBAY(JSUB,.,.,ICENTER)
        ...
        write (*,*) "YPRED", NOBSER, "of", MAXOBS, "updates"
 
	DO IOBS=1,NOBSER
	 DO IEQ=1,NUMEQT

         write (*,*) "YPREDBAY(,,,) update",JSUB
     1    ,IEQ,"of",NUMEQT
     2    ,IOBS,"of",MAXOBS
     3    ,ICENTER
...
c COMMON /OBSER/
        DIMENSION TIM(594),SIG(5000),RS(5000,34),YOO(594,MAXNUMEQ),
     1    BS(5000,7)
...
C--- ARG LIST
        ...
        REAL*8, dimension(20) :: VALFIX
        REAL*8 SIGFAC,OFAC
        REAL*8, dimension(30,2) :: AB
        CHARACTER PAR(30)*11
        integer NACTVE,NGRID
        REAL*8 VOLSPA
        REAL*8, dimension(32) :: IRAN
        REAL*8, dimension(3,30) :: CENTER
        REAL*8, dimension(MAXACT) :: PYJGXX
        integer NRANFIX
        REAL*8, dimension(20) ::  RANFIXEST
...
C wmy2017Sep19
        ...
        REAL*8, dimension(20) :: RCOPY,BCOPY
        REAL*8, dimension(30) :: EX,STD,COFVR,X
        REAL*8, dimension(30,30) :: COV,E,CORR
...
C        CALL IDPC(NPX,PX,W,NOBSER,NUMEQT)
            ...
            CALL IDPC(JSUB,IG,NPX,PX,W,NOBSER,NUMEQT,NDIM,MF,
     1        RTOL,ATOL,RSCOPY,BSCOPY,INTLIST,IPAR,ObsError,RPAR)

            write (*,*) "DO 7000 IDPC(JSUB,IG,W)",JSUB,IG,W
...
C
     ...
     1    NDIM,MF,RTOL,ATOL,PCOPY,RSCOPY,BSCOPY,INTLIST,IPAR,ObsError,
     2    RPAR)
