...
C New Code, Assumes Mix of ~ Poisson and ~ Normal
          ...
          IF(RPAR(128) .LE. 22708.D0) THEN
           IF(IG .LE. MAXACT) THEN
            PYJGX(JSUB,IG) = RPAR(129)
     1        * DEXP(-.5D0*RPAR(128))/RPAR(126)/RPAR(127)
           ENDIF
           WORKK(IG) = RPAR(129)
     1        * DEXP(-.5D0*RPAR(128))/RPAR(126)/RPAR(127)
          ENDIF
...
C          write (*,*) "TID", TID, "returns", work(IG), WORKK(IG) 
          ...
          write (*,*) " --- END DO 800 :: (JSUB,IG,s,w/w',PYJGX,px) ="
     1    ,ICYCLE,JSUB,IG,RPAR(126)
     1    ,W,RPAR(128),WORKK(IG),CORDEN(IG,NVAR+1)
...
C  ADD THIS W TO SUMTOT.
       ...
       SUMTOT = SUMTOT + W
C
C Note that SUMTOT = sum(z-score) over all observations, including
