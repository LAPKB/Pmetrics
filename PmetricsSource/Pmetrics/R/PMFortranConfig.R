...
PMFortranConfig <- function(reconfig=F){
  ...
  if(file.exists(configFilename)) {
    ...
    if(get("PmetricsBit",envir=PMenv)=="64" & length(compiler)==1 & length(grep("^gfortran",compiler)>0)){
      compiler <- c(compiler,"gfortran -O3 -w -fopenmp -fmax-stack-var-size=32768 -o <exec> <files>")  # -mcmodel=large failed :-(
  ...
  if(is.na(compiler[1]) | reconfig){
    ...
    cat(paste("1. gfortran -m",get("PmetricsBit",envir=PMenv)," -w -O3 -o <exec> <files>\ngfortran -O3 -w -fopenmp -fmax-stack-var-size=32768 -o <exec> <files>\n",sep=""))
