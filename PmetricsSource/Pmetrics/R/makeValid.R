...
#' @param icen Central tendency of parameter value distributions to be used for predictions.  Default is median.
#' @param binCov Character vector of covariates in model equations which should be binned.
#' @param clusters Numeric vector of length 2 with number of clusters to use for covariates in \code{binCov} and for time. If
#' not specified, Pmetrics will guide user through process to choose the number of clusters.
#' @param lower Lower percentile for pcVPC
#' @param upper Upper percentile for pcVPC 
#' @param \dots Other parameters to be passed to \code{\link{SIMrun}}, for example \code{limits}. 
#' @return The output of \code{makeValid} is a list of class \code{PMvalid} with the following objects
...
#' @seealso \code{\link{SIMrun}}, \code{\link{autonpde}}, \code{\link{plot.PMvalid}}
#' @references 
...
# Clean Up ----------------------------------------------------------------
  ...
  valRes <- list(upperDF=upperDF,medDF=medDF,lowerDF=lowerDF,quant_Obs=quant_Obs,
                 quant_pcObs=quant_pcObs,med2=med2,tempDF=tempDF,npde=npdeRes)
