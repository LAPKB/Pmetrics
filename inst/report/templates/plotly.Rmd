...
resid_plot <- function(x, y, icen, outeq){
  p <- res$op$data %>% 
    ...
    theme_classic() +
    labs(x = ifelse(deparse(substitute(x))=="time", "Time", "Observation"), 
         y = ifelse(deparse(substitute(y))=="d", "Prediction error", "Weighted Prediction Error"))
  return(p)
}
...
## Posterior residuals by observation {.tabset}
