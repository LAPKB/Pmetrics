...
```{r}
all_res <- nsub <- nobs <- numeqt <- icyctot <- NULL #declare for CRAN check
...
if(is.null(all_res$backend)){ #fortran
  nsub <- all_res$nsub
  icyctot <- all_res$icyctot
} else { #rust
  nsub <- all_res$final$nsub
  icyctot <- max(all_res$cycle$cycnum)
}



did_converge <- ifelse(all_res$converge | all_res$converge == 1, "converged.", "did not converge.")
...
After `r paste0(icyctot, " cycles, the model ", did_converge)`
...
par_lims <- data.frame(
  ...
  shrink = ifelse(!is.null(res$final$shrinkage), 100*t(res$final$shrinkage), NA)
