...
-   Use a previously created `PM_model` object, either from an appropriate file with `PM_model$new("filename")` but also by defining a model list directly in R  (see [models](models.html) for details) passing it to `PM_model$new()`.
...
```{r echo = TRUE, eval = FALSE, label = sim6}
