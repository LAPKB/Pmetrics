...
After a successful NPAG run, an R datafile is saved in the /outputs subdirectory of the newly created numerically ordered folder in the working directory. This file is called "PMout.Rdata". These data files can be loaded by ensuring that the Runs folder is set as the working directory, and then using the Pmetrics command `` `r gh_help("PM_load")` ``.
...
```{r echo = F, results = 'asis', label="obj_table"}
