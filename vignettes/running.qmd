...
```{r echo=T, eval=F, label=pm_fit_methods}
...
``` {r echo=T, eval=F, label=run_np_it}
library(Pmetrics)
...
``` {r echo=T, eval=F, label=run_examples}
...
``` {r echo=T, eval=F, label=legacy_plots}
plot(final.1)
plot(cycle.1)
...
``` {r echo=T, eval=F, label=legacy_run_example}
library(Pmetrics)
