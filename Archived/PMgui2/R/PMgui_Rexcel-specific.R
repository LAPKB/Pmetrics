...
#  last modified: 20 March 2008 by J. Fox  (following instructions from Erich Neuwirth)
    ...
    RExcelSupported <- function(){
    	RExcelSupport <- getOption("PMgui")$RExcelSupport
    ...
    SubmitToPmetrics <- function(commands){
        .log <- LogWindow()
        lines<-commands
        ...
        while (iline <= nlines){
            ...
            while (jline <= nlines){
                ...
                if (.console.output)cat(paste("PMgui+ ", lines[jline], sep="\n"))
                else{
...
##            else if (any(sapply(Pmetrics.Input.exceptions,
