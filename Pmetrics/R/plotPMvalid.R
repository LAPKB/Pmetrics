...
plot.PMvalid <- function(x, type = "vpc", tad = F, icen = "median", lower = 0.025, upper = 0.975,
  ...
  if (!tad) {
    ...
    if (!all(is.na(x$tadBinMedian))) {
      ...
      if (missing(xlab)) { xlab <- "Time after dose" }
  ...
  if (type == "vpc") {
    plotData <- list(obsQuant = quant_Obs, obs = x$opDF$obs, binTime = use.timeBinMedian,
                                    obsTime = use.optimes, upperDF = upperDF, lowerDF = lowerDF,
                                    medDF = medDF)
  }
  if (type == "pcvpc") {
    plotData <- list(obsQuant = quant_pcObs, obs = x$opDF$pcObs, binTime = use.timeBinMedian,
