...
makeValid <- function(run, outeq = 1, input = 1, icen = "median", tad = F, binCov, doseC, timeC, ...) {
  ...
  for (i in 1:nrow(mdata2)) {
    ...
    if (4 %in% missingVal) {
      #dose is missing
      if (i == 1 | (mdata2$id[i - 1] != mdata2$id[i])) {
        #first record for patient has zero dose
        j <- 0
        while (is.na(mdata2$dose[i + j])) {
          #increment until non-zero dose is found
          j <- j + 1
        }
  ...
  if (length(grep("IVERIFY", instrfile)) == 0) {
    #not updated instruction file
    modelfile <- readline("Your run used an old instruction file. Enter model name: ")
  } else {
    #ok we are using updated instruction file
