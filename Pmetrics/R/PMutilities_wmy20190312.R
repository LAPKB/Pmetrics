...
#
# BUG REPORT 2019.03.12 -- If model, data, and prior = OldRunNo
#   NPrun throws error:
#  R> NPrun(data=2,model=2,prior=2,run = 4)
#      Error in readLines(mod1, n = mod1ErrBlockStop) : 
#      vector size cannot be NA/NaN
#
# MN's fix was to skip this fix entirely and just tell user you
#   can not load old data to run with model containing new error poly.
#
...
#
deprecated_compareTwoModelERRs <- function (mod1,mod2)
{
  err.matches = F
...
parseBlocks <- function(model){
  ...
  if(length(grep("TSTMULT",modelFile))>0) {return(list(status=0,model=model))} #stop, we already have a fortran model file
...
makeModel <- function(model="model.txt",data="data.csv",engine,write=T,silent=F){
  ...
  pvardef <- psym
  if(length(grep(";",blocks$primVar))>0){ #using ';' as separator
    sep <- ";"
  } else {
    if(length(grep(",",blocks$primVar))>0){ #using ',' as separator
      sep <- ","
  ...
  if(engine$ncov>0) {ctype=rep(2,engine$ncov)} else {ctype=-99} #set covariate type based on number of covariates in data file, default is 2, interpolated
  ...
  if(engine$alg=="SIM"){
    ...
    if(length(gamlam)>0){ #gamma/lambda is specified in file
      ...
      if(engine$alg=="NP"){
        ...
        } else { #gamma/lambda are to be estimated
    ...
    } else { #gamma/lambda was omitted from file
  ...
  space <- function(n,x) {
    if(length(grep("^@",x))==0){ #not a preserve format line
  ...
  # analmodule <- list(header=NA,contains=NA) # wmy2018.10.07
...
# 6/11/18            "!$omp Threadprivate(/INPUT/)  ",
  ...
  if(length(grep("ifort",fortran)>0)){syntaxcheck <- paste(fortran,"/syntax-only",NPAGutils,modelFor) }
  if(length(grep("g95",fortran)>0)){syntaxcheck <- paste(fortran,"-fsyntax-only",NPAGutilsmodelFor) }
  ...
  if(engine$alg!="SIM" & write){
    ...
    if(!engine$wrkFlag){ #not using old working copy files
    ...
    } else {  #using old working copy files
    ...
    if(engine$alg=="NP"){ #create NPAG instruction file
    ...
    if(engine$alg=="IT" | engine$alg=="ERR"){ #create IT2B/ERR instruction file
...
kall_ijn<-function(Mu,c0,c1,c2,c3,nsubs,nout){
  ...
  skall<-matrix(0,nout,1); # to store norm as function of n=1:nout
  # fill Kall
  for (n in 1:nout){
    Kn<-kmat_ijn(Mu,n,c0,c1,c2,c3,nsubs,nout);
    skall[n]<-norm(Kn,"F"); # Frobenious norm
    Kall[ , ,n]<-Kn;
  }
...
perrorc1<- function(pH,Kall,nvec,Cbar){
  ...
  ExpKallsum0<-ExpKallsum-diag(diag(ExpKallsum)); # remove diagonal
